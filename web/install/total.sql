CREATE DATABASE NIMBUS_USER;

########################################################################################################

CREATE TABLE NIMBUS_USER.USER
(
	UID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	EMAIL VARCHAR(41) NOT NULL,
	PASSWD VARCHAR(41),
	FULLNAME NVARCHAR(41) NOT NULL,
	PIC MEDIUMBLOB,
	PIC_UP DATETIME,
	UNAME NVARCHAR(13),
	RTIME DATETIME NOT NULL,
	REG_TYPE ENUM('E', 'C') NOT NULL,
	DESCRIPTION TEXT,
	COMPANY TINYTEXT,
	TITLE TINYTEXT,
	CELL VARCHAR(15),
	WEIBO NVARCHAR(41),

	PRIMARY KEY (UID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

CREATE UNIQUE INDEX EMAIL_INDEX ON NIMBUS_USER.USER (EMAIL(40));
CREATE INDEX FULLNAME_INDEX ON NIMBUS_USER.USER (FULLNAME(40));
CREATE INDEX UNAME_INDEX ON NIMBUS_USER.USER (UNAME(12));

########################################################################################################

CREATE TABLE NIMBUS_USER.USER_T
(
	CID INT UNSIGNED NOT NULL,
	UID INT UNSIGNED NOT NULL,
	EMAIL VARCHAR(41),
	FULLNAME NVARCHAR(41) NOT NULL,
	PIC MEDIUMBLOB,
	PIC_UP DATETIME,
	CTIME DATETIME NOT NULL,
	DESCRIPTION TEXT,
	COMPANY TINYTEXT,
	TITLE TINYTEXT,
	CELL VARCHAR(15) NOT NULL,
	WEIBO NVARCHAR(41),
	
	FOREIGN KEY (UID) REFERENCES NIMBUS_USER.USER (UID),
	UNIQUE (CID, UID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

CREATE INDEX CID_INDEX ON NIMBUS_USER.USER_T (CID);
CREATE INDEX CTIME_INDEX ON NIMBUS_USER.USER_T (CTIME);

########################################################################################################

CREATE TABLE NIMBUS_USER.INVT
(
	ID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	CID INT UNSIGNED NOT NULL,
	UID INT UNSIGNED NOT NULL,
	CELL VARCHAR(15) NOT NULL,
	EMAIL VARCHAR(41),
	FULLNAME NVARCHAR(41) NOT NULL,
	COMPANY TINYTEXT,
	TITLE TINYTEXT,
	STIME DATETIME,
	
	PRIMARY KEY (ID),
	FOREIGN KEY (UID) REFERENCES NIMBUS_USER.USER (UID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

CREATE INDEX CID_INDEX ON NIMBUS_USER.INVT (CID);

########################################################################################################

CREATE TABLE NIMBUS_USER.CONTACT
(
	CID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	METHOD TINYTEXT NOT NULL,
	INFO TINYTEXT,
	UID INT UNSIGNED NOT NULL,

	PRIMARY KEY (CID),
	FOREIGN KEY (UID) REFERENCES NIMBUS_USER.USER (UID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

########################################################################################################

CREATE TABLE NIMBUS_USER.FOCUS
(
	UID INT UNSIGNED NOT NULL,
	FID INT UNSIGNED NOT NULL,

	FOREIGN KEY (UID) REFERENCES NIMBUS_USER.USER (UID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

########################################################################################################

CREATE TABLE NIMBUS_USER.FANS
(
	UID INT UNSIGNED NOT NULL,
	FID INT UNSIGNED NOT NULL,

	FOREIGN KEY (UID) REFERENCES NIMBUS_USER.USER (UID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

########################################################################################################

GRANT ALL ON NIMBUS_USER.* TO auser@'%' IDENTIFIED BY 'auserpass';
FLUSH PRIVILEGES;

CREATE DATABASE NIMBUS_CONF;

########################################################################################################

CREATE TABLE NIMBUS_CONF.CONF
(
	CID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	OWNER INT UNSIGNED NOT NULL,
	NAME NVARCHAR(41) NOT NULL,
	URL NVARCHAR(21) NOT NULL,
	IMG MEDIUMBLOB,
	IMG_UP DATETIME,
	BANNER MEDIUMBLOB,
	BANNER_UP DATETIME,
	COUN INT UNSIGNED NOT NULL,
	PROV INT UNSIGNED NOT NULL,
	CITY INT UNSIGNED NOT NULL,
	DIST INT UNSIGNED,
	ADDR NVARCHAR(41),
	FADDR NVARCHAR(61) NOT NULL,
	STIME DATETIME NOT NULL,
	REGISTRATION ENUM('O', 'C') NOT NULL,
	SEARCH ENUM('Y', 'N') NOT NULL,
	ETIME DATETIME,
	STATUS ENUM('D', 'P', 'C') NOT NULL,
	DESCRIPTION MEDIUMTEXT,
	CTIME DATETIME NOT NULL,
	LAT DECIMAL(12,8),
	LNG DECIMAL(12,8),

	PRIMARY KEY (CID),
	FOREIGN KEY (OWNER) REFERENCES NIMBUS_USER.USER (UID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

CREATE UNIQUE INDEX URL_INDEX ON NIMBUS_CONF.CONF (URL(20));
CREATE INDEX NAME_INDEX ON NIMBUS_CONF.CONF (NAME(40));
CREATE INDEX COUN_INDEX ON NIMBUS_CONF.CONF (COUN);
CREATE INDEX PROV_INDEX ON NIMBUS_CONF.CONF (PROV);
CREATE INDEX CITY_INDEX ON NIMBUS_CONF.CONF (CITY);
CREATE INDEX DIST_INDEX ON NIMBUS_CONF.CONF (DIST);
CREATE INDEX REGISTRATION_INDEX ON NIMBUS_CONF.CONF (REGISTRATION);
CREATE INDEX SEARCH_INDEX ON NIMBUS_CONF.CONF (SEARCH);
CREATE INDEX STATUS_INDEX ON NIMBUS_CONF.CONF (STATUS);
CREATE INDEX FADDR_INDEX ON NIMBUS_CONF.CONF (FADDR(60));
CREATE INDEX LAT_INDEX ON NIMBUS_CONF.CONF (LAT);
CREATE INDEX LNG_INDEX ON NIMBUS_CONF.CONF (LNG);

########################################################################################################

CREATE TABLE NIMBUS_CONF.SCHE
(
	SID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	CID INT UNSIGNED NOT NULL,
	UID INT UNSIGNED NOT NULL,
	DATE DATE NOT NULL,
	STIME TIME NOT NULL,
	ETIME TIME NOT NULL,
	LOCATION NVARCHAR(101) NOT NULL,
	SUMMARY NVARCHAR(101) NOT NULL,
	NOTE TEXT,

	PRIMARY KEY (SID),
	FOREIGN KEY (UID) REFERENCES NIMBUS_USER.USER (UID),
	FOREIGN KEY (CID) REFERENCES NIMBUS_CONF.CONF (CID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

########################################################################################################

CREATE TABLE NIMBUS_CONF.SPON
(
	SID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	CID INT UNSIGNED NOT NULL,
	CNAME NVARCHAR(41) NOT NULL,
	URL NVARCHAR(41),
	IMG MEDIUMBLOB,

	PRIMARY KEY (SID),	
	UNIQUE (CID, CNAME),
	FOREIGN KEY (CID) REFERENCES NIMBUS_CONF.CONF (CID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

CREATE INDEX CNAME_INDEX ON NIMBUS_CONF.SPON (CNAME(40));

########################################################################################################

CREATE TABLE NIMBUS_CONF.RELA
(
	ID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	UID INT UNSIGNED NOT NULL,
	CID INT UNSIGNED NOT NULL,
	ROLE ENUM('A', 'S') NOT NULL,

	PRIMARY KEY (ID),
	UNIQUE (CID, UID, ROLE),
	FOREIGN KEY (UID) REFERENCES NIMBUS_USER.USER (UID),
	FOREIGN KEY (CID) REFERENCES NIMBUS_CONF.CONF (CID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

########################################################################################################

GRANT ALL ON NIMBUS_CONF.* TO aconf@'%' IDENTIFIED BY 'aconfpass';
FLUSH PRIVILEGES;

CREATE DATABASE NIMBUS_NOTE;

########################################################################################################

CREATE TABLE NIMBUS_NOTE.NOTE
(
	ID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	UID INT UNSIGNED NOT NULL,
	CID INT UNSIGNED NOT NULL,
	NBODY TEXT,
	MDATA NVARCHAR(41),
	CTIME DATETIME NOT NULL,

	PRIMARY KEY (ID),
	FOREIGN KEY (UID) REFERENCES NIMBUS_USER.USER (UID),
	FOREIGN KEY (CID) REFERENCES NIMBUS_CONF.CONF (CID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

########################################################################################################

CREATE TABLE NIMBUS_NOTE.BROADCAST
(
	ID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	CID INT UNSIGNED NOT NULL,
	BTITLE NVARCHAR(41) NOT NULL,
	BBODY MEDIUMTEXT,
	CTIME DATETIME NOT NULL,
	UTIME DATETIME NOT NULL,

	PRIMARY KEY (ID),
	FOREIGN KEY (CID) REFERENCES NIMBUS_CONF.CONF (CID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

########################################################################################################

CREATE TABLE NIMBUS_NOTE.MESSAGE
(
	ID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	TITLE NVARCHAR(321) NOT NULL,

	R_ID INT UNSIGNED NOT NULL,
	S_ID INT UNSIGNED NOT NULL,

	BODY TEXT,

	CTIME DATETIME NOT NULL,
	IS_READ ENUM('N','Y'),

	PRIMARY KEY (ID),
	FOREIGN KEY (R_ID) REFERENCES NIMBUS_USER.USER (UID),
	FOREIGN KEY (S_ID) REFERENCES NIMBUS_USER.USER (UID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

CREATE INDEX TITLE_INDEX ON NIMBUS_NOTE.MESSAGE (TITLE(320));
CREATE INDEX IS_READ_INDEX ON NIMBUS_NOTE.MESSAGE (IS_READ);
CREATE INDEX CREATE_TIME_INDEX ON NIMBUS_NOTE.MESSAGE (CTIME);

########################################################################################################

GRANT ALL ON NIMBUS_NOTE.* TO anote@'%' IDENTIFIED BY 'anotepass';
FLUSH PRIVILEGES;

CREATE DATABASE NIMBUS_IPAD;

CREATE TABLE IF NOT EXISTS NIMBUS_IPAD.PROV
(
	ID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	PRONAME NVARCHAR(50) NOT NULL,
	PROVID INT UNSIGNED NOT NULL,
	PROREMARK NVARCHAR(50),
	PRIMARY KEY (ID),
	KEY (PROVID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

CREATE TABLE IF NOT EXISTS NIMBUS_IPAD.CITY
(
	ID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	CITYNAME NVARCHAR(51) NOT NULL,
	PROVID INT UNSIGNED NOT NULL,
	CITYID INT UNSIGNED NOT NULL,
	PRIMARY KEY (ID),
	KEY CITY_ID (CITYID),
	KEY PROV_ID (PROVID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

CREATE INDEX CITYNAME_INDEX ON NIMBUS_IPAD.CITY (CITYNAME(50));

CREATE TABLE IF NOT EXISTS NIMBUS_IPAD.DIST
(
	ID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	DISNAME NVARCHAR(30) NOT NULL,
	CITYID INT UNSIGNED NOT NULL,
	DISSORT INT UNSIGNED,
	PRIMARY KEY (ID),
	KEY CITY_ID (CITYID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

INSERT INTO NIMBUS_IPAD.PROV(PRONAME, PROVID, PROREMARK) VALUES
('北京','1','直辖市'),
('上海','9','直辖市'),
('天津','2','直辖市'),
('重庆','27','直辖市'),
('浙江','11','省份'),
('广东','19','省份'),
('湖南','18','省份'),
('湖北','17','省份'),
('四川','28','省份'),
('河北','3','省份'),
('山西','4','省份'),
('陕西','23','省份'),
('黑龙江','8','省份'),
('吉林','7','省份'),
('辽宁','6','省份'),
('内蒙古','5','自治区'),
('河南','16','省份'),
('山东','15','省份'),
('江苏','10','省份'),
('安徽','12','省份'),
('福建','13','省份'),
('江西','14','省份'),
('海南','20','省份'),
('广西','21','自治区'),
('甘肃','22','省份'),
('新疆','24','自治区'),
('青海','25','省份'),
('宁夏','26','自治区'),
('贵州','29','省份'),
('云南','30','省份'),
('西藏','31','自治区'),
('台湾','32','省份'),
('澳门','33','特别行政区'),
('香港','34','特别行政区');

INSERT INTO NIMBUS_IPAD.CITY(CITYNAME, PROVID, CITYID) VALUES
('北京市','1','1'),
('天津市','2','2'),
('上海市','9','3'),
('重庆市','27','4'),
('石家庄市','3','6'),
('邯郸市','3','5'),
('保定市','3','7'),
('张家口市','3','8'),
('承德市','3','9'),
('唐山市','3','10'),
('廊坊市','3','11'),
('沧州市','3','12'),
('衡水市','3','13'),
('邢台市','3','14'),
('秦皇岛市','3','15'),
('太原市','4','18'),
('大同市','4','19'),
('朔州市','4','16'),
('忻州市','4','17'),
('阳泉市','4','20'),
('晋中市','4','21'),
('长治市','4','22'),
('晋城市','4','23'),
('临汾市','4','24'),
('吕梁市','4','25'),
('运城市','4','26'),
('沈阳市','6','27'),
('铁岭市','6','28'),
('大连市','6','29'),
('鞍山市','6','30'),
('抚顺市','6','31'),
('本溪市','6','32'),
('丹东市','6','33'),
('锦州市','6','34'),
('营口市','6','35'),
('阜新市','6','36'),
('辽阳市','6','37'),
('朝阳市','6','38'),
('盘锦市','6','39'),
('葫芦岛市','6','40'),
('长春市','7','41'),
('吉林市','7','42'),
('延边朝鲜族自治州','7','43'),
('四平市','7','44'),
('通化市','7','45'),
('白城市','7','46'),
('辽源市','7','47'),
('松原市','7','48'),
('白山市','7','49'),
('哈尔滨市','8','50'),
('齐齐哈尔市','8','51'),
('鸡西市','8','52'),
('牡丹江市','8','53'),
('七台河市','8','54'),
('佳木斯市','8','55'),
('鹤岗市','8','56'),
('双鸭山市','8','57'),
('绥化市','8','58'),
('黑河市','8','59'),
('大兴安岭地区','8','60'),
('伊春市','8','61'),
('大庆市','8','62'),
('南京市','10','63'),
('无锡市','10','64'),
('镇江市','10','65'),
('苏州市','10','66'),
('南通市','10','67'),
('扬州市','10','68'),
('盐城市','10','69'),
('徐州市','10','70'),
('淮安市','10','71'),
('连云港市','10','72'),
('常州市','10','73'),
('泰州市','10','74'),
('宿迁市','10','75'),
('杭州市','11','78'),
('宁波市','11','81'),
('绍兴市','11','82'),
('舟山市','11','76'),
('衢州市','11','77'),
('湖州市','11','79'),
('嘉兴市','11','80'),
('温州市','11','83'),
('丽水市','11','84'),
('金华市','11','85'),
('台州市','11','86'),
('合肥市','12','87'),
('芜湖市','12','88'),
('蚌埠市','12','89'),
('淮南市','12','90'),
('马鞍山市','12','91'),
('淮北市','12','92'),
('铜陵市','12','93'),
('安庆市','12','94'),
('黄山市','12','95'),
('滁州市','12','96'),
('阜阳市','12','97'),
('宿州市','12','98'),
('巢湖市','12','99'),
('六安市','12','100'),
('亳州市','12','101'),
('池州市','12','102'),
('福州市','13','104'),
('宣城市','12','103'),
('厦门市','13','105'),
('宁德市','13','106'),
('莆田市','13','107'),
('泉州市','13','108'),
('漳州市','13','109'),
('龙岩市','13','110'),
('三明市','13','111'),
('南平市','13','112'),
('南昌市','14','115'),
('鹰潭市','14','113'),
('新余市','14','114'),
('九江市','14','116'),
('上饶市','14','117'),
('抚州市','14','118'),
('宜春市','14','119'),
('吉安市','14','120'),
('赣州市','14','121'),
('景德镇市','14','122'),
('萍乡市','14','123'),
('济南市','15','125'),
('青岛市','15','126'),
('菏泽市','15','124'),
('淄博市','15','127'),
('德州市','15','128'),
('烟台市','15','129'),
('潍坊市','15','130'),
('济宁市','15','131'),
('泰安市','15','132'),
('临沂市','15','133'),
('滨州市','15','134'),
('东营市','15','135'),
('威海市','15','136'),
('枣庄市','15','137'),
('日照市','15','138'),
('莱芜市','15','139'),
('聊城市','15','140'),
('郑州市','16','142'),
('新乡市','16','144'),
('商丘市','16','141'),
('安阳市','16','143'),
('许昌市','16','145'),
('平顶山市','16','146'),
('信阳市','16','147'),
('南阳市','16','148'),
('开封市','16','149'),
('洛阳市','16','150'),
('济源市','16','151'),
('焦作市','16','152'),
('鹤壁市','16','153'),
('濮阳市','16','154'),
('周口市','16','155'),
('漯河市','16','156'),
('驻马店市','16','157'),
('三门峡市','16','158'),
('武汉市','17','159'),
('襄樊市','17','160'),
('鄂州市','17','161'),
('孝感市','17','162'),
('黄冈市','17','163'),
('黄石市','17','164'),
('咸宁市','17','165'),
('荆州市','17','166'),
('宜昌市','17','167'),
('恩施土家族苗族自治州','17','168'),
('神农架林区','17','169'),
('十堰市','17','170'),
('随州市','17','171'),
('荆门市','17','172'),
('仙桃市','17','173'),
('天门市','17','174'),
('潜江市','17','175'),
('长沙市','18','177'),
('岳阳市','18','176'),
('湘潭市','18','178'),
('株洲市','18','179'),
('衡阳市','18','180'),
('郴州市','18','181'),
('常德市','18','182'),
('益阳市','18','183'),
('娄底市','18','184'),
('邵阳市','18','185'),
('湘西土家族苗族自治州','18','186'),
('张家界市','18','187'),
('怀化市','18','188'),
('永州市','18','189'),
('广州市','19','190'),
('汕尾市','19','191'),
('阳江市','19','192'),
('揭阳市','19','193'),
('茂名市','19','194'),
('惠州市','19','195'),
('江门市','19','196'),
('韶关市','19','197'),
('梅州市','19','198'),
('汕头市','19','199'),
('深圳市','19','200'),
('珠海市','19','201'),
('佛山市','19','202'),
('肇庆市','19','203'),
('湛江市','19','204'),
('中山市','19','205'),
('河源市','19','206'),
('清远市','19','207'),
('云浮市','19','208'),
('潮州市','19','209'),
('东莞市','19','210'),
('兰州市','22','211'),
('金昌市','22','212'),
('白银市','22','213'),
('天水市','22','214'),
('嘉峪关市','22','215'),
('武威市','22','216'),
('张掖市','22','217'),
('平凉市','22','218'),
('酒泉市','22','219'),
('庆阳市','22','220'),
('定西市','22','221'),
('陇南市','22','222'),
('临夏回族自治州','22','223'),
('甘南藏族自治州','22','224'),
('成都市','28','225'),
('攀枝花市','28','226'),
('自贡市','28','227'),
('绵阳市','28','228'),
('南充市','28','229'),
('达州市','28','230'),
('遂宁市','28','231'),
('广安市','28','232'),
('巴中市','28','233'),
('泸州市','28','234'),
('宜宾市','28','235'),
('资阳市','28','236'),
('内江市','28','237'),
('乐山市','28','238'),
('眉山市','28','239'),
('凉山彝族自治州','28','240'),
('雅安市','28','241'),
('甘孜藏族自治州','28','242'),
('阿坝藏族羌族自治州','28','243'),
('德阳市','28','244'),
('广元市','28','245'),
('贵阳市','29','246'),
('遵义市','29','247'),
('安顺市','29','248'),
('黔南布依族苗族自治州','29','249'),
('黔东南苗族侗族自治州','29','250'),
('铜仁地区','29','251'),
('毕节地区','29','252'),
('六盘水市','29','253'),
('黔西南布依族苗族自治州','29','254'),
('海口市','20','255'),
('三亚市','20','256'),
('五指山市','20','257'),
('琼海市','20','258'),
('儋州市','20','259'),
('文昌市','20','260'),
('万宁市','20','261'),
('东方市','20','262'),
('澄迈县','20','263'),
('定安县','20','264'),
('屯昌县','20','265'),
('临高县','20','266'),
('白沙黎族自治县','20','267'),
('昌江黎族自治县','20','268'),
('乐东黎族自治县','20','269'),
('陵水黎族自治县','20','270'),
('保亭黎族苗族自治县','20','271'),
('琼中黎族苗族自治县','20','272'),
('昆明市','30','276'),
('西双版纳傣族自治州','30','273'),
('德宏傣族景颇族自治州','30','274'),
('昭通市','30','275'),
('大理白族自治州','30','277'),
('红河哈尼族彝族自治州','30','278'),
('曲靖市','30','279'),
('保山市','30','280'),
('文山壮族苗族自治州','30','281'),
('玉溪市','30','282'),
('楚雄彝族自治州','30','283'),
('普洱市','30','284'),
('临沧市','30','285'),
('怒江傈傈族自治州','30','286'),
('迪庆藏族自治州','30','287'),
('丽江市','30','288'),
('西宁市','25','290'),
('海北藏族自治州','25','289'),
('海东地区','25','291'),
('黄南藏族自治州','25','292'),
('海南藏族自治州','25','293'),
('果洛藏族自治州','25','294'),
('玉树藏族自治州','25','295'),
('海西蒙古族藏族自治州','25','296'),
('西安市','23','297'),
('咸阳市','23','298'),
('延安市','23','299'),
('榆林市','23','300'),
('渭南市','23','301'),
('商洛市','23','302'),
('安康市','23','303'),
('汉中市','23','304'),
('宝鸡市','23','305'),
('铜川市','23','306'),
('南宁市','21','308'),
('防城港市','21','307'),
('崇左市','21','309'),
('来宾市','21','310'),
('柳州市','21','311'),
('桂林市','21','312'),
('梧州市','21','313'),
('贺州市','21','314'),
('贵港市','21','315'),
('玉林市','21','316'),
('百色市','21','317'),
('钦州市','21','318'),
('河池市','21','319'),
('北海市','21','320'),
('拉萨市','31','321'),
('日喀则地区','31','322'),
('山南地区','31','323'),
('林芝地区','31','324'),
('昌都地区','31','325'),
('那曲地区','31','326'),
('阿里地区','31','327'),
('银川市','26','328'),
('石嘴山市','26','329'),
('吴忠市','26','330'),
('固原市','26','331'),
('中卫市','26','332'),
('乌鲁木齐市','24','340'),
('塔城地区','24','333'),
('哈密地区','24','334'),
('和田地区','24','335'),
('阿勒泰地区','24','336'),
('克孜勒苏柯尔克孜自治州','24','337'),
('博尔塔拉蒙古自治州','24','338'),
('克拉玛依市','24','339'),
('石河子市','24','341'),
('昌吉回族自治州','24','342'),
('五家渠市','24','343'),
('吐鲁番地区','24','344'),
('巴音郭楞蒙古自治州','24','345'),
('阿克苏地区','24','346'),
('阿拉尔市','24','347'),
('喀什地区','24','348'),
('图木舒克市','24','349'),
('伊犁哈萨克自治州','24','350'),
('包头市','5','353'),
('呼伦贝尔市','5','351'),
('呼和浩特市','5','352'),
('乌海市','5','354'),
('乌兰察布市','5','355'),
('通辽市','5','356'),
('赤峰市','5','357'),
('鄂尔多斯市','5','358'),
('巴彦淖尔市','5','359'),
('锡林郭勒盟','5','360'),
('兴安盟','5','361'),
('阿拉善盟','5','362'),
('台北市','32','363'),
('高雄市','32','364'),
('基隆市','32','365'),
('台中市','32','366'),
('台南市','32','367'),
('新竹市','32','368'),
('嘉义市','32','369'),
('澳门特别行政区','33','370'),
('香港特别行政区','34','371');

INSERT INTO NIMBUS_IPAD.DIST(DISNAME, CITYID) VALUES
--  1 北京
('东城区','1'),
('西城区','1'),
('崇文区','1'),
('宣武区','1'),
('朝阳区','1'),
('丰台区','1'),
('石景山区','1'),
('海淀区','1'),
('门头沟区','1'),
('房山区','1'),
('通州区','1'),
('顺义区','1'),
('昌平区','1'),
('大兴区','1'),
('怀柔区','1'),
('平谷区','1'),
('密云县','1'),
('延庆县','1'),

-- 2 天津
('和平区','2'),
('河东区','2'),
('河西区','2'),
('南开区','2'),
('河北区','2'),
('红桥区','2'),
('塘沽区','2'),
('汉沽区','2'),
('大港区','2'),
('东丽区','2'),
('西青区','2'),
('津南区','2'),
('北辰区','2'),
('武清区','2'),
('宝坻区','2'),
('宁河县','2'),
('静海县','2'),
('蓟县','2'),

-- 3 上海市
('黄浦区','3'),
('卢湾区','3'),
('徐汇区','3'),
('长宁区','3'),
('静安区','3'),
('普陀区','3'),
('闸北区','3'),
('虹口区','3'),
('杨浦区','3'),
('闵行区','3'),
('宝山区','3'),
('嘉定区','3'),
('浦东新区','3'),
('金山区','3'),
('松江区','3'),
('青浦区','3'),
('南汇区','3'),
('奉贤区','3'),
('崇明县','3'),

-- 4 重庆市
('万州区','4'),
('涪陵区','4'),
('渝中区','4'),
('大渡口区','4'),
('江北区','4'),
('沙坪坝区','4'),
('九龙坡区','4'),
('南岸区','4'),
('北碚区','4'),
('万盛区','4'),
('双桥区','4'),
('渝北区','4'),
('巴南区','4'),
('黔江区','4'),
('长寿区','4'),
('江津区','4'),
('合川区','4'),
('永川区','4'),
('南川区','4'),
('綦江县','4'),
('潼南县','4'),
('铜梁县','4'),
('大足县','4'),
('荣昌县','4'),
('璧山县','4'),
('梁平县','4'),
('城口县','4'),
('丰都县','4'),
('垫江县','4'),
('武隆县','4'),
('忠县','4'),
('开县','4'),
('云阳县','4'),
('奉节县','4'),
('巫山县','4'),
('巫溪县','4'),
('石柱土家族自治县','4'),
('秀山土家族苗族自治县','4'),
('酉阳土家族苗族自治县','4'),
('彭水苗族土家族自治县','4'),

-- ****************************************************************河北省*****************************************************************
-- 5 邯郸市
('邯山区','5'),
('丛台区','5'),
('复兴区','5'),
('峰峰矿区','5'),
('邯郸县','5'),
('临漳县','5'),
('成安县','5'),
('大名县','5'),
('涉县','5'),
('磁县','5'),
('肥乡县','5'),
('永年县','5'),
('邱县','5'),
('鸡泽县','5'),
('广平县','5'),
('馆陶县','5'),
('魏县','5'),
('曲周县','5'),
('武安市','5'),

-- 6 石家庄市
('长安区','6'),
('桥东区','6'),
('桥西区','6'),
('新华区','6'),
('井陉矿区','6'),
('裕华区','6'),
('井陉县','6'),
('正定县','6'),
('栾城县','6'),
('行唐县','6'),
('灵寿县','6'),
('高邑县','6'),
('深泽县','6'),
('赞皇县','6'),
('无极县','6'),
('平山县','6'),
('元氏县','6'),
('赵县','6'),
('辛集市','6'),
('藁城市','6'),
('晋州市','6'),
('新乐市','6'),
('鹿泉市','6'),

-- 7 保定市
('新市区','7'),
('北市区','7'),
('南市区','7'),
('满城县','7'),
('清苑县','7'),
('涞水县','7'),
('阜平县','7'),
('徐水县','7'),
('定兴县','7'),
('唐县','7'),
('高阳县','7'),
('容城县','7'),
('涞源县','7'),
('望都县','7'),
('安新县','7'),
('易县','7'),
('曲阳县','7'),
('蠡县','7'),
('顺平县','7'),
('博野县','7'),
('雄县','7'),
('涿州市','7'),
('定州市','7'),
('安国市','7'),
('高碑店市','7'),

-- 8 张家口市
('桥东区','8'),
('桥西区','8'),
('宣化区','8'),
('下花园区','8'),
('宣化县','8'),
('张北县','8'),
('康保县','8'),
('沽源县','8'),
('尚义县','8'),
('蔚县','8'),
('阳原县','8'),
('怀安县','8'),
('万全县','8'),
('怀来县','8'),
('涿鹿县','8'),
('赤城县','8'),
('崇礼县','8'),

-- 9 承德市
('双桥区','9'),
('双滦区','9'),
('鹰手营子矿区','9'),
('承德县','9'),
('兴隆县','9'),
('平泉县','9'),
('滦平县','9'),
('隆化县','9'),
('丰宁满族自治县','9'),
('宽城满族自治县','9'),
('围场满族蒙古族自治县','9'),

-- 10 唐山市
('路南区','10'),
('路北区','10'),
('古冶区','10'),
('开平区','10'),
('丰南区','10'),
('丰润区','10'),
('滦县','10'),
('滦南县','10'),
('乐亭县','10'),
('迁西县','10'),
('玉田县','10'),
('唐海县','10'),
('遵化市','10'),
('迁安市','10'),
 
-- 11 廊坊市
('安次区','11'),
('广阳区','11'),
('固安县','11'),
('永清县','11'),
('香河县','11'),
('大城县','11'),
('文安县','11'),
('大厂回族自治县','11'),
('霸州市','11'),
('三河市','11'),

-- 12 沧州市
('新华区','12'),
('运河区','12'),
('沧县','12'),
('青县','12'),
('东光县','12'),
('海兴县','12'),
('盐山县','12'),
('肃宁县','12'),
('南皮县','12'),
('吴桥县','12'),
('献县','12'),
('孟村回族自治县','12'),
('泊头市','12'),
('任丘市','12'),
('黄骅市','12'),
('河间市','12'),

-- 13 衡水市
('桃城区','13'),
('枣强县','13'),
('武邑县','13'),
('武强县','13'),
('饶阳县','13'),
('安平县','13'),
('故城县','13'),
('景县','13'),
('阜城县','13'),
('冀州市','13'),
('深州市','13'),


-- 14 邢台市
('桥东区','14'),
('桥西区','14'),
('邢台县','14'),
('临城县','14'),
('内丘县','14'),
('柏乡县','14'),
('隆尧县','14'),
('任县','14'),
('南和县','14'),
('宁晋县','14'),
('巨鹿县','14'),
('新河县','14'),
('广宗县','14'),
('平乡县','14'),
('威县','14'),
('清河县','14'),
('临西县','14'),
('南宫市','14'),
('沙河市','14'),

-- 15 秦皇岛市
('海港区','15'),
('山海关区','15'),
('北戴河区','15'),
('青龙满族自治县','15'),
('昌黎县','15'),
('抚宁县','15'),
('卢龙县','15'),


-- *********************************************************山西************************************************************************
-- 16 朔州市
('朔城区','16'),
('平鲁区','16'),
('山阴县','16'),
('应县','16'),
('右玉县','16'),
('怀仁县','16'),

-- 17 忻州市
('忻府区','17'),
('定襄县','17'),
('五台县','17'),
('代县','17'),
('繁峙县','17'),
('宁武县','17'),
('静乐县','17'),
('神池县','17'),
('五寨县','17'),
('岢岚县','17'),
('河曲县','17'),
('保德县','17'),
('偏关县','17'),
('原平市','17'),

-- 18 太原市
('小店区','18'),
('迎泽区','18'),
('杏花岭区','18'),
('尖草坪区','18'),
('万柏林区','18'),
('晋源区','18'),
('清徐县','18'),
('阳曲县','18'),
('娄烦县','18'),
('古交市','18'),

-- 19 大同市
('矿区','19'),
('南郊区','19'),
('新荣区','19'),
('阳高县','19'),
('天镇县','19'),
('广灵县','19'),
('灵丘县','19'),
('浑源县','19'),
('左云县','19'),
('大同县','19'),

-- 20 阳泉市
('矿区','20'),
('平定县','20'),
('盂县','20'),

-- 21 晋中市
('榆次区','21'),
('榆社县','21'),
('左权县','21'),
('和顺县','21'),
('昔阳县','21'),
('寿阳县','21'),
('太谷县','21'),
('祁县','21'),
('平遥县','21'),
('灵石县','21'),
('介休市','21'),

-- 22 长治市
('长治县','22'),
('襄垣县','22'),
('屯留县','22'),
('平顺县','22'),
('黎城县','22'),
('壶关县','22'),
('长子县','22'),
('武乡县','22'),
('沁县','22'),
('沁源县','22'),
('潞城市','22'),

-- 23 晋城市
('沁水县','23'),
('阳城县','23'),
('陵川县','23'),
('泽州县','23'),
('高平市','23'),

-- 24 临汾市
('尧都区','24'),
('曲沃县','24'),
('翼城县','24'),
('襄汾县','24'),
('洪洞县','24'),
('古县','24'),
('安泽县','24'),
('浮山县','24'),
('吉县','24'),
('乡宁县','24'),
('大宁县','24'),
('隰县','24'),
('永和县','24'),
('蒲县','24'),
('汾西县','24'),
('侯马市','24'),
('霍州市','24'),

-- 25 吕梁市
('离石区','25'),
('文水县','25'),
('交城县','25'),
('兴县','25'),
('临县','25'),
('柳林县','25'),
('石楼县','25'),
('岚县','25'),
('方山县','25'),
('中阳县','25'),
('交口县','25'),
('孝义市','25'),
('汾阳市','25'),

-- 26 运城市
('盐湖区','26'),
('临猗县','26'),
('万荣县','26'),
('闻喜县','26'),
('稷山县','26'),
('新绛县','26'),
('绛县','26'),
('垣曲县','26'),
('夏县','26'),
('平陆县','26'),
('芮城县','26'),
('永济市','26'),
('河津市','26'),

 
-- *********************************************************辽宁************************************************************************
-- 27 沈阳市
('和平区','27'),
('沈河区','27'),
('大东区','27'),
('皇姑区','27'),
('铁西区','27'),
('苏家屯区','27'),
('东陵区','27'),
('沈北新区','27'),
('于洪区','27'),
('辽中县','27'),
('康平县','27'),
('法库县','27'),
('新民市','27'),

-- 28 铁岭市
('银州区','28'),
('清河区','28'),
('铁岭县','28'),
('西丰县','28'),
('昌图县','28'),
('调兵山市','28'),
('开原市','28'),


-- 29 大连市
('长海县','29'),
('旅顺口区','29'),
('中山区','29'),
('西岗区','29'),
('沙河口区','29'),
('甘井子区','29'),
('金州区','29'),
('普兰店市','29'),
('瓦房店市','29'),
('庄河市','29'),

-- 30 鞍山市
('铁东区','30'),
('铁西区','30'),
('立山区','30'),
('千山区','30'),
('台安县','30'),
('岫岩满族自治县','30'),
('海城市','30'),

-- 31 抚顺市
('新抚区','31'),
('东洲区','31'),
('望花区','31'),
('顺城区','31'),
('抚顺县','31'),
('新宾满族自治县','31'),
('清原满族自治县','31'),


-- 32 本溪市
('平山区','32'),
('溪湖区','32'),
('明山区','32'),
('南芬区','32'),
('本溪满族自治县','32'),
('桓仁满族自治县','32'),

-- 33 丹东市
('元宝区','33'),
('振兴区','33'),
('振安区','33'),
('宽甸满族自治县','33'),
('东港市','33'),
('凤城市','33'),


-- 34 锦州市
('古塔区','34'),
('凌河区','34'),
('太和区','34'),
('黑山县','34'),
('义县','34'),
('凌海市','34'),
('北镇市','34'),


-- 35 营口市
('站前区','35'),
('西市区','35'),
('鮁鱼圈区','35'),
('老边区','35'),
('盖州市','35'),
('大石桥市','35'),


-- 36 阜新市
('海州区','36'),
('新邱区','36'),
('太平区','36'),
('清河门区','36'),
('细河区','36'),
('阜新蒙古族自治县','36'),
('彰武县','36'),


-- 37 辽阳市
('白塔区','37'),
('文圣区','37'),
('宏伟区','37'),
('弓长岭区','37'),
('太子河区','37'),
('辽阳县','37'),
('灯塔市','37'),


-- 38 朝阳市
('双塔区','38'),
('龙城区','38'),
('朝阳县','38'),
('建平县','38'),
('喀喇沁左翼蒙古族自治县','38'),
('北票市','38'),
('凌源市','38'),


-- 39 盘锦市
('双台子区','39'),
('兴隆台区','39'),
('大洼县','39'),
('盘山县','39'),

-- 40 葫芦岛市
('连山区','40'),
('龙港区','40'),
('南票区','40'),
('绥中县','40'),
('建昌县','40'),
('兴城市','40'),


-- *********************************************************吉林************************************************************************
-- 41 长春市
('南关区','41'),
('宽城区','41'),
('朝阳区','41'),
('二道区','41'),
('绿园区','41'),
('双阳区','41'),
('农安县','41'),
('九台市','41'),
('榆树市','41'),
('德惠市','41'),


-- 42 吉林市
('昌邑区','42'),
('龙潭区','42'),
('船营区','42'),
('丰满区','42'),
('永吉县','42'),
('蛟河市','42'),
('桦甸市','42'),
('舒兰市','42'),
('磐石市','42'),


-- 43 延边朝鲜族自治州
('延吉市','43'),
('图们市','43'),
('敦化市','43'),
('珲春市','43'),
('龙井市','43'),
('和龙市','43'),
('汪清县','43'),
('安图县','43'),


-- 44 四平市
('铁西区','44'),
('铁东区','44'),
('梨树县','44'),
('伊通满族自治县','44'),
('公主岭市','44'),
('双辽市','44'),


-- 45 通化市
('东昌区','45'),
('二道江区','45'),
('通化县','45'),
('辉南县','45'),
('柳河县','45'),
('梅河口市','45'),
('集安市','45'),


-- 46 白城市
('洮北区','46'),
('镇赉县','46'),
('通榆县','46'),
('洮南市','46'),
('大安市','46'),


-- 47 辽源市
('龙山区','47'),
('西安区','47'),
('东丰县','47'),
('东辽县','47'),


-- 48 松原市
('宁江区','48'),
('前郭尔罗斯蒙古族自治县','48'),
('长岭县','48'),
('乾安县','48'),
('扶余县','48'),


-- 49 白山市
('八道江区','49'),
('江源区','49'),
('抚松县','49'),
('靖宇县','49'),
('长白朝鲜族自治县','49'),
('临江市','49'),


-- *********************************************************黑龙江************************************************************************
-- 50 哈尔滨市
('道里区','50'),
('南岗区','50'),
('道外区','50'),
('平房区','50'),
('松北区','50'),
('香坊区','50'),
('呼兰区','50'),
('阿城区','50'),
('依兰县','50'),
('方正县','50'),
('宾县','50'),
('巴彦县','50'),
('木兰县','50'),
('通河县','50'),
('延寿县','50'),
('双城市','50'),
('尚志市','50'),
('五常市','50'),

-- 51 齐齐哈尔市
('龙沙区','51'),
('建华区','51'),
('铁锋区','51'),
('昂昂溪区','51'),
('富拉尔基区','51'),
('碾子山区','51'),
('梅里斯达翰尔族区','51'),
('龙江县','51'),
('依安县','51'),
('泰来县','51'),
('甘南县','51'),
('富裕县','51'),
('克山县','51'),
('克东县','51'),
('拜泉县','51'),
('讷河市','51'),

-- 52 鸡西市
('鸡冠区','52'),
('恒山区','52'),
('滴道区','52'),
('梨树区','52'),
('城子河区','52'),
('麻山区','52'),
('鸡东县','52'),
('虎林市','52'),
('密山市','52'),

-- 53 牡丹江市
('东安区','53'),
('阳明区','53'),
('爱民区','53'),
('西安区','53'),
('东宁县','53'),
('林口县','53'),
('绥芬河市','53'),
('海林市','53'),
('宁安市','53'),
('穆棱市','53'),

-- 54 七台河市
('新兴区','54'),
('桃山区','54'),
('茄子河区','54'),
('勃利县','54'),


-- 55 佳木斯市
('向阳区','55'),
('前进区','55'),
('东风区','55'),
('桦南县','55'),
('桦川县','55'),
('汤原县','55'),
('抚远县','55'),
('同江市','55'),
('富锦市','55'),


-- 56 鹤岗市
('向阳区','56'),
('工农区','56'),
('南山区','56'),
('兴安区','56'),
('东山区','56'),
('兴山区','56'),
('萝北县','56'),
('绥滨县','56'),


-- 57 双鸭山市
('尖山区','57'),
('岭东区','57'),
('四方台区','57'),
('宝山区','57'),
('集贤县','57'),
('友谊县','57'),
('宝清县','57'),
('饶河县','57'),


-- 58 绥化市
('北林区','58'),
('望奎县','58'),
('兰西县','58'),
('青冈县','58'),
('庆安县','58'),
('明水县','58'),
('绥棱县','58'),
('安达市','58'),
('肇东市','58'),
('海伦市','58'),

-- 59 黑河市
('爱辉区','59'),
('嫩江县','59'),
('逊克县','59'),
('孙吴县','59'),
('北安市','59'),
('五大连池市','59'),

-- 60 大兴安岭地区
('呼玛县','60'),
('塔河县','60'),
('漠河县','60'),

-- 61 伊春市
('伊春区','61'),
('南岔区','61'),
('友好区','61'),
('西林区','61'),
('翠峦区','61'),
('新青区','61'),
('美溪区','61'),
('金山屯区','61'),
('五营区','61'),
('乌马河区','61'),
('汤旺河区','61'),
('带岭区','61'),
('乌伊岭区','61'),
('红星区','61'),
('上甘岭区','61'),
('嘉荫县','61'),
('铁力市','61'),


-- 62 大庆市
('萨尔图区','62'),
('龙凤区','62'),
('让胡路区','62'),
('红岗区','62'),
('大同区','62'),
('肇州县','62'),
('肇源县','62'),
('林甸县','62'),
('杜尔伯特蒙古族自治县','62'),

-- *********************************************************江苏************************************************************************
-- 63 南京市
('江宁区','63'),
('浦口区','63'),
('玄武区','63'),
('白下区','63'),
('秦淮区','63'),
('建邺区','63'),
('鼓楼区','63'),
('下关区','63'),
('栖霞区','63'),
('雨花台区','63'),
('六合区','63'),
('溧水县','63'),
('高淳县','63'),

-- 64 无锡市
('崇安区','64'),
('南长区','64'),
('北塘区','64'),
('锡山区','64'),
('惠山区','64'),
('滨湖区','64'),
('江阴市','64'),
('宜兴市','64'),


-- 65 镇江市
('京口区','65'),
('润州区','65'),
('丹徒区','65'),
('丹阳市','65'),
('扬中市','65'),
('句容市','65'),

-- 66 苏州市
('沧浪区','66'),
('常熟市','66'),
('平江区','66'),
('金阊区','66'),
('虎丘区','66'),
('昆山市','66'),
('太仓市','66'),
('吴江市','66'),
('吴中区','66'),
('相城区','66'),
('张家港市','66'),

-- 67 南通市
('崇川区','67'),
('港闸区','67'),
('海安县','67'),
('如东县','67'),
('启东市','67'),
('如皋市','67'),
('通州市','67'),
('海门市','67'),

-- 68 扬州市
('高邮市','68'),
('广陵区','68'),
('邗江区','68'),
('维扬区','68'),
('宝应县','68'),
('江都市','68'),
('仪征市','68'),

-- 69 盐城市
('亭湖区','69'),
('盐都区','69'),
('响水县','69'),
('滨海县','69'),
('阜宁县','69'),
('射阳县','69'),
('建湖县','69'),
('东台市','69'),
('大丰市','69'),

-- 70 徐州市
('鼓楼区','70'),
('云龙区','70'),
('九里区','70'),
('贾汪区','70'),
('泉山区','70'),
('丰县','70'),
('沛县','70'),
('铜山县','70'),
('睢宁县','70'),
('新沂市','70'),
('邳州市','70'),

-- 71 淮安市
('清河区','71'),
('楚州区','71'),
('淮阴区','71'),
('清浦区','71'),
('涟水县','71'),
('洪泽县','71'),
('盱眙县','71'),
('金湖县','71'),


-- 72 连云港市
('连云区','72'),
('新浦区','72'),
('海州区','72'),
('赣榆县','72'),
('东海县','72'),
('灌云县','72'),
('灌南县','72'),


-- 73 常州市
('天宁区','73'),
('钟楼区','73'),
('戚墅堰区','73'),
('新北区','73'),
('武进区','73'),
('溧阳市','73'),
('金坛市','73'),

-- 74 泰州市
('海陵区','74'),
('高港区','74'),
('兴化市','74'),
('靖江市','74'),
('泰兴市','74'),
('姜堰市','74'),


-- 75 宿迁市
('宿城区','75'),
('宿豫区','75'),
('沭阳县','75'),
('泗阳县','75'),
('泗洪县','75'),

-- *********************************************************浙江************************************************************************
-- 76 舟山市
('定海区','76'),
('普陀区','76'),
('岱山县','76'),
('嵊泗县','76'),

-- 77 衢州市
('柯城区','77'),
('衢江区','77'),
('常山县','77'),
('开化县','77'),
('龙游县','77'),
('江山市','77'),


-- 78 杭州市
('上城区','78'),
('下城区','78'),
('江干区','78'),
('拱墅区','78'),
('西湖区','78'),
('滨江区','78'),
('余杭区','78'),
('桐庐县','78'),
('淳安县','78'),
('建德市','78'),
('富阳市','78'),
('临安市','78'),
('萧山区','78'),

-- 79 湖州市
('吴兴区','79'),
('南浔区','79'),
('德清县','79'),
('长兴县','79'),
('安吉县','79'),

-- 80 嘉兴市
(' 南湖区','80'),
(' 秀洲区','80'),
(' 嘉善县','80'),
(' 海盐县','80'),
(' 海宁市','80'),
(' 平湖市','80'),
(' 桐乡市 ','80'),


-- 81 宁波市
('海曙区','81'),
('江东区','81'),
('江北区','81'),
('北仑区','81'),
('镇海区','81'),
('鄞州区','81'),
('象山县','81'),
('宁海县','81'),
('余姚市','81'),
('慈溪市','81'),
('奉化市','81'),


-- 82 绍兴市
('越城区','82'),
('绍兴县','82'),
('新昌县','82'),
('诸暨市','82'),
('上虞市','82'),
('嵊州市','82'),


-- 83 温州市
('鹿城区','83'),
('龙湾区','83'),
('瓯海区','83'),
('洞头县','83'),
('永嘉县','83'),
('平阳县','83'),
('苍南县','83'),
('文成县','83'),
('泰顺县','83'),
('瑞安市','83'),
('乐清市','83'),


-- 84 丽水市
('莲都区','84'),
('青田县','84'),
('缙云县','84'),
('遂昌县','84'),
('松阳县','84'),
('云和县','84'),
('庆元县','84'),
('景宁畲族自治县','84'),
('龙泉市','84'),


-- 85 金华市
('婺城区','85'),
('金东区','85'),
('武义县','85'),
('浦江县','85'),
('磐安县','85'),
('兰溪市','85'),
('义乌市','85'),
('东阳市','85'),
('永康市','85'),

-- 86 台州市
('椒江区','86'),
('黄岩区','86'),
('路桥区','86'),
('玉环县','86'),
('三门县','86'),
('天台县','86'),
('仙居县','86'),
('温岭市','86'),
('临海市','86'),



-- *********************************************************安徽************************************************************************
-- 87 合肥市
('瑶海区','87'),
('庐阳区','87'),
('蜀山区','87'),
('包河区','87'),
('长丰县','87'),
('肥东县','87'),
('肥西县','87'),

-- 88 芜湖市
('镜湖区','88'),
('弋江区','88'),
('鸠江区','88'),
('三山区','88'),
('芜湖县','88'),
('繁昌县','88'),
('南陵县','88'),

-- 89 蚌埠市
('龙子湖区','89'),
('蚌山区','89'),
('禹会区','89'),
('淮上区','89'),
('怀远县','89'),
('五河县','89'),
('固镇县','89'),


-- 90 淮南市
('大通区','90'),
('田家庵区','90'),
('谢家集区','90'),
('八公山区','90'),
('潘集区','90'),
('凤台县','90'),


-- 91 马鞍山市
('金家庄区','91'),
('花山区','91'),
('雨山区','91'),
('当涂县','91'),


-- 92 淮北市
('杜集区','92'),
('相山区','92'),
('烈山区','92'),
('濉溪县 ','92'),


-- 93 铜陵市
('铜官山区','93'),
('狮子山区','93'),
('铜陵县','93'),


-- 94 安庆市
('迎江区','94'),
('大观区','94'),
('宜秀区','94'),
('怀宁县','94'),
('枞阳县','94'),
('潜山县','94'),
('太湖县','94'),
('宿松县','94'),
('望江县','94'),
('岳西县','94'),
('桐城市','94'),


-- 95 黄山市
('屯溪区','95'),
('黄山区','95'),
('徽州区','95'),
('歙县','95'),
('休宁县','95'),
('黟县','95'),
('祁门县','95'),


-- 96 滁州市
('琅琊区','96'),
('南谯区','96'),
('来安县','96'),
('全椒县','96'),
('定远县','96'),
('凤阳县','96'),
('天长市','96'),
('明光市','96'),


-- 97 阜阳市
('颍州区','97'),
('颍东区','97'),
('颍泉区','97'),
('临泉县','97'),
('太和县','97'),
('阜南县','97'),
('颍上县','97'),
('界首市','97'),


-- 98 宿州市
('埇桥区','98'),
('砀山县','98'),
('萧县','98'),
('灵璧县','98'),
('泗县 ','98'),


-- 99 巢湖市
('居巢区','99'),
('庐江县','99'),
('无为县','99'),
('含山县','99'),
('和县 ','99'),


-- 100 六安市
('金安区','100'),
('裕安区','100'),
('寿县','100'),
('霍邱县','100'),
('舒城县','100'),
('金寨县','100'),
('霍山县','100'),


-- 101 亳州市
('谯城区','101'),
('涡阳县','101'),
('蒙城县','101'),
('利辛县','101'),


-- 102 池州市
('贵池区','102'),
('东至县','102'),
('石台县','102'),
('青阳县','102'),


-- 103 宣城市
('宣州区','103'),
('郎溪县','103'),
('广德县','103'),
('泾县','103'),
('绩溪县','103'),
('旌德县','103'),
('宁国市','103'),



-- *********************************************************福建************************************************************************
-- 104 福州市
('鼓楼区','104'),
('台江区','104'),
('仓山区','104'),
('马尾区','104'),
('晋安区','104'),
('闽侯县','104'),
('连江县','104'),
('罗源县','104'),
('闽清县','104'),
('永泰县','104'),
('平潭县','104'),
('福清市','104'),
('长乐市','104'),


-- 105 厦门市
('思明区','105'),
('海沧区','105'),
('湖里区','105'),
('集美区','105'),
('同安区','105'),
('翔安区','105'),


-- 106 宁德市
('蕉城区','106'),
('霞浦县','106'),
('古田县','106'),
('屏南县','106'),
('寿宁县','106'),
('周宁县','106'),
('柘荣县','106'),
('福安市','106'),
('福鼎市','106'),


-- 107 莆田市
('城厢区','107'),
('涵江区','107'),
('荔城区','107'),
('秀屿区','107'),
('仙游县','107'),


-- 108 泉州市
('鲤城区','108'),
('丰泽区','108'),
('洛江区','108'),
('泉港区','108'),
('惠安县','108'),
('安溪县','108'),
('永春县','108'),
('德化县','108'),
('石狮市','108'),
('晋江市','108'),
('南安市','108'),


-- 109 漳州市
('芗城区','109'),
('龙文区','109'),
('云霄县','109'),
('漳浦县','109'),
('诏安县','109'),
('长泰县','109'),
('东山县','109'),
('南靖县','109'),
('平和县','109'),
('华安县','109'),
('龙海市','109'),

-- 110 龙岩市
('新罗区','110'),
('长汀县','110'),
('永定县','110'),
('上杭县','110'),
('武平县','110'),
('连城县','110'),
('漳平市','110'),

-- 111 三明市
('梅列区','111'),
('三元区','111'),
('明溪县','111'),
('清流县','111'),
('宁化县','111'),
('大田县','111'),
('尤溪县','111'),
('沙县','111'),
('将乐县','111'),
('泰宁县','111'),
('建宁县','111'),
('永安市','111'),


-- 112 南平市
('延平区','112'),
('顺昌县','112'),
('浦城县','112'),
('光泽县','112'),
('松溪县','112'),
('政和县','112'),
('邵武市','112'),
('武夷山市','112'),
('建瓯市','112'),
('建阳市','112'),

-- *********************************************************江西************************************************************************
-- 113 鹰潭市
('月湖区','113'),
('余江县','113'),
('贵溪市','113'),

-- 114 新余市
('渝水区','114'),
('分宜县','114'),


-- 115 南昌市
('东湖区','115'),
('西湖区','115'),
('青云谱区','115'),
('湾里区','115'),
('青山湖区','115'),
('南昌县','115'),
('新建县','115'),
('安义县','115'),
('进贤县','115'),


-- 116 九江市
('庐山区','116'),
('浔阳区','116'),
('九江县','116'),
('武宁县','116'),
('修水县','116'),
('永修县','116'),
('德安县','116'),
('星子县','116'),
('都昌县','116'),
('湖口县','116'),
('彭泽县','116'),
('瑞昌市','116'),

-- 117 上饶市
('信州区','117'),
('上饶县','117'),
('广丰县','117'),
('玉山县','117'),
('铅山县','117'),
('横峰县','117'),
('弋阳县','117'),
('余干县','117'),
('鄱阳县','117'),
('万年县','117'),
('婺源县','117'),
('德兴市','117'),

-- 118 抚州市
('临川区','118'),
('南城县','118'),
('黎川县','118'),
('南丰县','118'),
('崇仁县','118'),
('乐安县','118'),
('宜黄县','118'),
('金溪县','118'),
('资溪县','118'),
('东乡县','118'),
('广昌县','118'),

-- 119 宜春市
('袁州区','119'),
('奉新县','119'),
('万载县','119'),
('上高县','119'),
('宜丰县','119'),
('靖安县','119'),
('铜鼓县','119'),
('丰城市','119'),
('樟树市','119'),
('高安市','119'),


-- 120 吉安市
('吉州区','120'),
('青原区','120'),
('吉安县','120'),
('吉水县','120'),
('峡江县','120'),
('新干县','120'),
('永丰县','120'),
('泰和县','120'),
('遂川县','120'),
('万安县','120'),
('安福县','120'),
('永新县','120'),
('井冈山市','120'),


-- 121 赣州市
('章贡区','121'),
('赣县','121'),
('信丰县','121'),
('大余县','121'),
('上犹县','121'),
('崇义县','121'),
('安远县','121'),
('龙南县','121'),
('定南县','121'),
('全南县','121'),
('宁都县','121'),
('于都县','121'),
('兴国县','121'),
('会昌县','121'),
('寻乌县','121'),
('石城县','121'),
('瑞金市','121'),
('南康市','121'),

-- 122 景德镇市
('昌江区','122'),
('珠山区','122'),
('浮梁县','122'),
('乐平市','122'),


-- 123 萍乡市
('安源区','123'),
('湘东区','123'),
('莲花县','123'),
('上栗县','123'),
('芦溪县','123'),

-- *********************************************************山东************************************************************************
-- 124 菏泽市
('牡丹区','124'),
('曹县','124'),
('单县','124'),
('成武县','124'),
('巨野县','124'),
('郓城县','124'),
('鄄城县','124'),
('定陶县','124'),
('东明县','124'),


-- 125 济南市
('历下区','125'),
('市中区','125'),
('槐荫区','125'),
('天桥区','125'),
('历城区','125'),
('长清区','125'),
('平阴县','125'),
('济阳县','125'),
('商河县','125'),
('章丘市','125'),

-- 126 青岛市
('市南区','126'),
('市北区','126'),
('四方区','126'),
('黄岛区','126'),
('崂山区','126'),
('李沧区','126'),
('城阳区','126'),
('胶州市','126'),
('即墨市','126'),
('平度市','126'),
('胶南市','126'),
('莱西市','126'),


-- 127 淄博市
('淄川区','127'),
('张店区','127'),
('博山区','127'),
('临淄区','127'),
('周村区','127'),
('桓台县','127'),
('高青县','127'),
('沂源县','127'),

-- 128 德州市
('德城区','128'),
('陵县','128'),
('宁津县','128'),
('庆云县','128'),
('临邑县','128'),
('齐河县','128'),
('平原县','128'),
('夏津县','128'),
('武城县','128'),
('乐陵市','128'),
('禹城市','128'),

-- 129 烟台市
('芝罘区','129'),
('福山区','129'),
('牟平区','129'),
('莱山区','129'),
('长岛县','129'),
('龙口市','129'),
('莱阳市','129'),
('莱州市','129'),
('蓬莱市','129'),
('招远市','129'),
('栖霞市','129'),
('海阳市','129'),

-- 130 潍坊市
('潍城区','130'),
('寒亭区','130'),
('坊子区','130'),
('奎文区','130'),
('临朐县','130'),
('昌乐县','130'),
('青州市','130'),
('诸城市','130'),
('寿光市','130'),
('安丘市','130'),
('高密市','130'),
('昌邑市','130'),

-- 131 济宁市
('市中区','131'),
('任城区','131'),
('微山县','131'),
('鱼台县','131'),
('金乡县','131'),
('嘉祥县','131'),
('汶上县','131'),
('泗水县','131'),
('梁山县','131'),
('曲阜市','131'),
('兖州市','131'),
('邹城市','131'),

-- 132 泰安市
('泰山区','132'),
('岱岳区','132'),
('宁阳县','132'),
('东平县','132'),
('新泰市','132'),
('肥城市','132'),

-- 133 临沂市
('兰山区','133'),
('罗庄区','133'),
('河东区','133'),
('沂南县','133'),
('郯城县','133'),
('沂水县','133'),
('苍山县','133'),
('费县','133'),
('平邑县','133'),
('莒南县','133'),
('蒙阴县','133'),
('临沭县','133'),


-- 134 滨州市
('滨城区','134'),
('惠民县','134'),
('阳信县','134'),
('无棣县','134'),
('沾化县','134'),
('博兴县','134'),
('邹平县','134'),

-- 135 东营市
('东营区','135'),
('河口区','135'),
('垦利县','135'),
('利津县','135'),
('广饶县','135'),


-- 136 威海市
('环翠区','136'),
('文登市','136'),
('荣成市','136'),
('乳山市','136'),

-- 137 枣庄市
('市中区','137'),
('薛城区','137'),
('峄城区','137'),
('台儿庄区','137'),
('山亭区','137'),
('滕州市','137'),

-- 138 日照市
('东港区','138'),
('岚山区','138'),
('五莲县','138'),
('莒县','138'),


-- 139 莱芜市
('莱城区','139'),
('钢城区','139'),


-- 140 聊城市
('东昌府区','140'),
('阳谷县','140'),
('莘县','140'),
('茌平县','140'),
('东阿县','140'),
('冠县','140'),
('高唐县','140'),
('临清市','140'),


-- *********************************************************河南************************************************************************
-- 141 商丘市
('梁园区','141'),
('睢阳区','141'),
('民权县','141'),
('睢县','141'),
('宁陵县','141'),
('柘城县','141'),
('虞城县','141'),
('夏邑县','141'),
('永城市','141'),

-- 142 郑州市
('中原区','142'),
('二七区','142'),
('管城回族区','142'),
('金水区','142'),
('上街区','142'),
('惠济区','142'),
('中牟县','142'),
('巩义市','142'),
('荥阳市','142'),
('新密市','142'),
('新郑市','142'),
('登封市','142'),

-- 143 安阳市
('文峰区','143'),
('北关区','143'),
('殷都区','143'),
('龙安区','143'),
('安阳县','143'),
('汤阴县','143'),
('滑县','143'),
('内黄县','143'),
('林州市','143'),


-- 144 新乡市
('红旗区','144'),
('卫滨区','144'),
('凤泉区','144'),
('牧野区','144'),
('新乡县','144'),
('获嘉县','144'),
('原阳县','144'),
('延津县','144'),
('封丘县','144'),
('长垣县','144'),
('卫辉市','144'),
('辉县市','144'),


-- 145 许昌市
('魏都区','145'),
('许昌县','145'),
('鄢陵县','145'),
('襄城县','145'),
('禹州市','145'),
('长葛市','145'),


-- 146 平顶山市
('新华区','146'),
('卫东区','146'),
('石龙区','146'),
('湛河区','146'),
('宝丰县','146'),
('叶县','146'),
('鲁山县','146'),
('郏县','146'),
('舞钢市','146'),
('汝州市','146'),

-- 147 信阳市
('浉河区','147'),
('平桥区','147'),
('罗山县','147'),
('光山县','147'),
('新县','147'),
('商城县','147'),
('固始县','147'),
('潢川县','147'),
('淮滨县','147'),
('息县','147'),


-- 148 南阳市
('宛城区','148'),
('卧龙区','148'),
('南召县','148'),
('方城县','148'),
('西峡县','148'),
('镇平县','148'),
('内乡县','148'),
('淅川县','148'),
('社旗县','148'),
('唐河县','148'),
('新野县','148'),
('桐柏县','148'),
('邓州市','148'),

-- 149 开封市
('龙亭区','149'),
('顺河回族区','149'),
('鼓楼区','149'),
('禹王台区','149'),
('金明区','149'),
('杞县','149'),
('通许县','149'),
('尉氏县','149'),
('开封县','149'),
('兰考县','149'),


-- 150 洛阳市
('老城区','150'),
('西工区','150'),
('瀍河回族区','150'),
('涧西区','150'),
('吉利区','150'),
('洛龙区','150'),
('孟津县','150'),
('新安县','150'),
('栾川县','150'),
('嵩县','150'),
('汝阳县','150'),
('宜阳县','150'),
('洛宁县','150'),
('伊川县','150'),
('偃师市','150'),


-- 151 济源市(无)
-- 152 焦作市
('解放区','152'),
('中站区','152'),
('马村区','152'),
('山阳区','152'),
('修武县','152'),
('博爱县','152'),
('武陟县','152'),
('温县','152'),
('沁阳市','152'),
('孟州市','152'),


-- 153 鹤壁市
('鹤山区','153'),
('山城区','153'),
('淇滨区','153'),
('浚县','153'),
('淇县','153'),

-- 154 濮阳市
('华龙区','154'),
('清丰县','154'),
('南乐县','154'),
('范县','154'),
('台前县','154'),
('濮阳县','154'),

-- 155 周口市
('川汇区','155'),
('扶沟县','155'),
('西华县','155'),
('商水县','155'),
('沈丘县','155'),
('郸城县','155'),
('淮阳县','155'),
('太康县','155'),
('鹿邑县','155'),
('项城市','155'),


-- 156 漯河市
('源汇区','156'),
('郾城区','156'),
('召陵区','156'),
('舞阳县','156'),
('临颍县','156'),

-- 157 驻马店市
('驿城区','157'),
('西平县','157'),
('上蔡县','157'),
('平舆县','157'),
('正阳县','157'),
('确山县','157'),
('泌阳县','157'),
('汝南县','157'),
('遂平县','157'),
('新蔡县','157'),

-- 158 三门峡市
('湖滨区','158'),
('渑池县','158'),
('陕县','158'),
('卢氏县','158'),
('义马市','158'),
('灵宝市','158'),

-- *********************************************************湖北************************************************************************
-- 159 武汉市
('江岸区','159'),
('江汉区','159'),
('硚口区','159'),
('汉阳区','159'),
('武昌区','159'),
('青山区','159'),
('洪山区','159'),
('东西湖区','159'),
('汉南区','159'),
('蔡甸区','159'),
('江夏区','159'),
('黄陂区','159'),
('新洲区','159'),


-- 160 襄樊市
('襄城区','160'),
('樊城区','160'),
('襄阳区','160'),
('南漳县','160'),
('谷城县','160'),
('保康县','160'),
('老河口市','160'),
('枣阳市','160'),
('宜城市','160'),


-- 161 鄂州市
('梁子湖区','161'),
('华容区','161'),
('鄂城区','161'),


-- 162 孝感市
('孝南区','162'),
('孝昌县','162'),
('大悟县','162'),
('云梦县','162'),
('应城市','162'),
('安陆市','162'),
('汉川市','162'),


-- 163 黄冈市
('黄州区','163'),
('团风县','163'),
('红安县','163'),
('罗田县','163'),
('英山县','163'),
('浠水县','163'),
('蕲春县','163'),
('黄梅县','163'),
('麻城市','163'),
('武穴市','163'),


-- 164 黄石市
('黄石港区','164'),
('西塞山区','164'),
('下陆区','164'),
('铁山区','164'),
('阳新县','164'),
('大冶市','164'),

-- 165 咸宁市
('咸安区','165'),
('嘉鱼县','165'),
('通城县','165'),
('崇阳县','165'),
('通山县','165'),
('赤壁市','165'),

-- 166 荆州市
('沙市区','166'),
('荆州区','166'),
('公安县','166'),
('监利县','166'),
('江陵县','166'),
('石首市','166'),
('洪湖市','166'),
('松滋市','166'),


-- 167 宜昌市
('西陵区','167'),
('伍家岗区','167'),
('点军区','167'),
('猇亭区','167'),
('夷陵区','167'),
('远安县','167'),
('兴山县','167'),
('秭归县','167'),
('长阳土家族自治县','167'),
('五峰土家族自治县','167'),
('宜都市','167'),
('当阳市','167'),
('枝江市','167'),


-- 168 恩施土家族苗族自治州
('恩施市','168'),
('利川市','168'),
('建始县','168'),
('巴东县','168'),
('宣恩县','168'),
('咸丰县','168'),
('来凤县','168'),
('鹤峰县','168'),


-- 169 神农架林区(无)
-- 170 十堰市
('茅箭区','170'),
('张湾区','170'),
('郧县','170'),
('郧西县','170'),
('竹山县','170'),
('竹溪县','170'),
('房县','170'),
('丹江口市','170'),


-- 171 随州市
('曾都区','171'),
('广水市','171'),


-- 172 荆门市
('东宝区','172'),
('掇刀区','172'),
('京山县','172'),
('沙洋县','172'),
('钟祥市','172'),


-- 173 仙桃市(无)
-- 174 天门市(无)
-- 175 潜江市(无)

-- *********************************************************湖南************************************************************************
-- 176 岳阳市
('岳阳楼区','176'),
('云溪区','176'),
('君山区','176'),
('岳阳县','176'),
('华容县','176'),
('湘阴县','176'),
('平江县','176'),
('汨罗市','176'),
('临湘市','176'),


-- 177 长沙市
('芙蓉区','177'),
('天心区','177'),
('岳麓区','177'),
('开福区','177'),
('雨花区','177'),
('长沙县','177'),
('望城县','177'),
('宁乡县','177'),
('浏阳市','177'),

-- 178 湘潭市
('雨湖区','178'),
('岳塘区','178'),
('湘潭县','178'),
('湘乡市','178'),
('韶山市','178'),


-- 179 株洲市
('荷塘区','179'),
('芦淞区','179'),
('石峰区','179'),
('天元区','179'),
('株洲县','179'),
('攸县','179'),
('茶陵县','179'),
('炎陵县','179'),
('醴陵市','179'),

-- 180 衡阳市
('珠晖区','180'),
('雁峰区','180'),
('石鼓区','180'),
('蒸湘区','180'),
('南岳区','180'),
('衡阳县','180'),
('衡南县','180'),
('衡山县','180'),
('衡东县','180'),
('祁东县','180'),
('耒阳市','180'),
('常宁市','180'),

-- 181 郴州市
('北湖区','181'),
('苏仙区','181'),
('桂阳县','181'),
('宜章县','181'),
('永兴县','181'),
('嘉禾县','181'),
('临武县','181'),
('汝城县','181'),
('桂东县','181'),
('安仁县','181'),
('资兴市','181'),

-- 182 常德市
('武陵区','182'),
('鼎城区','182'),
('安乡县','182'),
('汉寿县','182'),
('澧县','182'),
('临澧县','182'),
('桃源县','182'),
('石门县','182'),
('津市市','182'),

-- 183 益阳市
('资阳区','183'),
('赫山区','183'),
('南县','183'),
('桃江县','183'),
('安化县','183'),
('沅江市','183'),

-- 184 娄底市
('娄星区','184'),
('双峰县','184'),
('新化县','184'),
('冷水江市','184'),
('涟源市','184'),


-- 185 邵阳市
('双清区','185'),
('大祥区','185'),
('北塔区','185'),
('邵东县','185'),
('新邵县','185'),
('邵阳县','185'),
('隆回县','185'),
('洞口县','185'),
('绥宁县','185'),
('新宁县','185'),
('城步苗族自治县','185'),
('武冈市','185'),

-- 186 湘西土家族苗族自治州
('吉首市','186'),
('泸溪县','186'),
('凤凰县','186'),
('花垣县','186'),
('保靖县','186'),
('古丈县','186'),
('永顺县','186'),
('龙山县','186'),


-- 187 张家界市
('永定区','187'),
('武陵源区','187'),
('慈利县','187'),
('桑植县','187'),

-- 188 怀化市
('鹤城区','188'),
('中方县','188'),
('沅陵县','188'),
('辰溪县','188'),
('溆浦县','188'),
('会同县','188'),
('麻阳苗族自治县','188'),
('新晃侗族自治县','188'),
('芷江侗族自治县','188'),
('靖州苗族侗族自治县','188'),
('通道侗族自治县','188'),
('洪江市','188'),

-- 189 永州市
('零陵区','189'),
('冷水滩区','189'),
('祁阳县','189'),
('东安县','189'),
('双牌县','189'),
('道县','189'),
('江永县','189'),
('宁远县','189'),
('蓝山县','189'),
('新田县','189'),
('江华瑶族自治县','189'),

-- *********************************************************广东************************************************************************
-- 190 广州市
('从化市','190'),
('荔湾区','190'),
('越秀区','190'),
('海珠区','190'),
('天河区','190'),
('白云区','190'),
('花都区','190'),
('黄埔区','190'),
('萝岗区','190'),
('南沙区','190'),
('番禺区','190'),
('增城市','190'),

-- 191 汕尾市
('海丰县','191'),
('陆河县','191'),
('陆丰市','191'),


-- 192 阳江市
('江城区','192'),
('阳西县','192'),
('阳东县','192'),
('阳春市','192'),


-- 193 揭阳市
('榕城区','193'),
('揭东县','193'),
('揭西县','193'),
('惠来县','193'),
('普宁市','193'),


-- 194 茂名市
('茂南区','194'),
('茂港区','194'),
('电白县','194'),
('高州市','194'),
('化州市','194'),
('信宜市','194'),


-- 195 惠州市
('惠城区','195'),
('惠阳区','195'),
('博罗县','195'),
('惠东县','195'),
('龙门县','195'),


-- 196 江门市
('蓬江区','196'),
('江海区','196'),
('新会区','196'),
('台山市','196'),
('开平市','196'),
('鹤山市','196'),
('恩平市','196'),

-- 197 韶关市
('武江区','197'),
('浈江区','197'),
('曲江区','197'),
('始兴县','197'),
('仁化县','197'),
('翁源县','197'),
('乳源瑶族自治县','197'),
('新丰县','197'),
('乐昌市','197'),
('南雄市','197'),


-- 198 梅州市
('梅江区','198'),
('梅县','198'),
('大埔县','198'),
('丰顺县','198'),
('五华县','198'),
('平远县','198'),
('蕉岭县','198'),
('兴宁市','198'),

-- 199 汕头市
('龙湖区','199'),
('金平区','199'),
('濠江区','199'),
('潮阳区','199'),
('潮南区','199'),
('澄海区','199'),
('南澳县','199'),


-- 200 深圳市
('罗湖区','200'),
('福田区','200'),
('南山区','200'),
('宝安区','200'),
('龙岗区','200'),
('盐田区','200'),


-- 201 珠海市
('香洲区','201'),
('斗门区','201'),
('金湾区','201'),


-- 202 佛山市
('禅城区','202'),
('南海区','202'),
('顺德区','202'),
('三水区','202'),
('高明区','202'),


-- 203 肇庆市
('端州区','203'),
('鼎湖区','203'),
('广宁县','203'),
('怀集县','203'),
('封开县','203'),
('德庆县','203'),
('高要市','203'),
('四会市','203'),


-- 204 湛江市
('赤坎区','204'),
('霞山区','204'),
('坡头区','204'),
('麻章区','204'),
('遂溪县','204'),
('徐闻县','204'),
('廉江市','204'),
('雷州市','204'),
('吴川市','204'),


-- 205 中山市(无)
-- 206 河源市
('源城区','206'),
('紫金县','206'),
('龙川县','206'),
('连平县','206'),
('和平县','206'),
('东源县','206'),

-- 207 清远市
('清城区','207'),
('佛冈县','207'),
('阳山县','207'),
('连山壮族瑶族自治县','207'),
('连南瑶族自治县','207'),
('清新县','207'),
('英德市','207'),
('连州市','207'),

-- 208 云浮市
('云城区','208'),
('新兴县','208'),
('郁南县','208'),
('云安县','208'),
('罗定市','208'),

-- 209 潮州市
('湘桥区','209'),
('潮安县','209'),
('饶平县','209'),

-- 210 东莞市(无)

-- *********************************************************甘肃************************************************************************
-- 211 兰州市
('城关区','211'),
('七里河区','211'),
('西固区','211'),
('安宁区','211'),
('红古区','211'),
('永登县','211'),
('皋兰县','211'),
('榆中县','211'),

-- 212 金昌市
('金川区','212'),
('永昌县','212'),

-- 213 白银市
('白银区','213'),
('平川区','213'),
('靖远县','213'),
('会宁县','213'),
('景泰县','213'),

-- 214 天水市
('秦州区','214'),
('麦积区','214'),
('清水县','214'),
('秦安县','214'),
('甘谷县','214'),
('武山县','214'),
('张家川回族自治县','214'),

-- 215 嘉峪关市(无)
-- 216 武威市
('凉州区','216'),
('民勤县','216'),
('古浪县','216'),
('天祝藏族自治县','216'),

-- 217 张掖市
('甘州区','217'),
('肃南裕固族自治县','217'),
('民乐县','217'),
('临泽县','217'),
('高台县','217'),
('山丹县','217'),

-- 218 平凉市
('崆峒区','218'),
('泾川县','218'),
('灵台县','218'),
('崇信县','218'),
('华亭县','218'),
('庄浪县','218'),
('静宁县','218'),

-- 219 酒泉市
('肃州区','219'),
('金塔县','219'),
('瓜州县','219'),
('肃北蒙古族自治县','219'),
('阿克塞哈萨克族自治县','219'),
('玉门市','219'),
('敦煌市','219'),

-- 220 庆阳市
('西峰区','220'),
('庆城县','220'),
('环县','220'),
('华池县','220'),
('合水县','220'),
('正宁县','220'),
('宁县','220'),
('镇原县','220'),

-- 221 定西市
('安定区','221'),
('通渭县','221'),
('陇西县','221'),
('渭源县','221'),
('临洮县','221'),
('漳县','221'),
('岷县','221'),

-- 222 陇南市
('武都区','222'),
('成县','222'),
('文县','222'),
('宕昌县','222'),
('康县','222'),
('西和县','222'),
('礼县','222'),
('徽县','222'),
('两当县','222'),

-- 223 临夏回族自治州
('临夏市','223'),
('临夏县','223'),
('康乐县','223'),
('永靖县','223'),
('广河县','223'),
('和政县','223'),
('东乡族自治县','223'),
('积石山保安族东乡族撒拉族自治县','223'),

-- 224 甘南藏族自治州
('合作市','224'),
('临潭县','224'),
('卓尼县','224'),
('舟曲县','224'),
('迭部县','224'),
('玛曲县','224'),
('碌曲县','224'),
('夏河县','224'),

-- *********************************************************四川************************************************************************
-- 225 成都市
('锦江区','225'),
('青羊区','225'),
('金牛区','225'),
('武侯区','225'),
('成华区','225'),
('龙泉驿区','225'),
('青白江区','225'),
('新都区','225'),
('温江区','225'),
('金堂县','225'),
('双流县','225'),
('郫县','225'),
('大邑县','225'),
('蒲江县','225'),
('新津县','225'),
('都江堰市','225'),
('彭州市','225'),
('邛崃市','225'),
('崇州市','225'),

-- 226 攀枝花市
('东区','226'),
('西区','226'),
('仁和区','226'),
('米易县','226'),
('盐边县','226'),

-- 227 自贡市
('自流井区','227'),
('贡井区','227'),
('大安区','227'),
('沿滩区','227'),
('荣县','227'),
('富顺县','227'),

-- 228 绵阳市
('涪城区','228'),
('游仙区','228'),
('三台县','228'),
('盐亭县','228'),
('安县','228'),
('梓潼县','228'),
('北川羌族自治县','228'),
('平武县','228'),
('江油市','228'),

-- 229 南充市
('顺庆区','229'),
('高坪区','229'),
('嘉陵区','229'),
('南部县','229'),
('营山县','229'),
('蓬安县','229'),
('仪陇县','229'),
('西充县','229'),
('阆中市','229'),

-- 230 达州市
('通川区','230'),
('达县','230'),
('宣汉县','230'),
('开江县','230'),
('大竹县','230'),
('渠县','230'),
('万源市','230'),

-- 231 遂宁市
('船山区','231'),
('安居区','231'),
('蓬溪县','231'),
('射洪县','231'),
('大英县','231'),


-- 232 广安市
('广安区','232'),
('岳池县','232'),
('武胜县','232'),
('邻水县','232'),
('华蓥市','232'),

-- 233 巴中市
('巴州区','233'),
('通江县','233'),
('南江县','233'),
('平昌县','233'),

-- 234 泸州市
('江阳区','234'),
('纳溪区','234'),
('龙马潭区','234'),
('泸县','234'),
('合江县','234'),
('叙永县','234'),
('古蔺县','234'),

-- 235 宜宾市
('翠屏区','235'),
('宜宾县','235'),
('南溪县','235'),
('江安县','235'),
('长宁县','235'),
('高县','235'),
('珙县','235'),
('筠连县','235'),
('兴文县','235'),
('屏山县','235'),

-- 236 资阳市
('雁江区','236'),
('安岳县','236'),
('乐至县','236'),
('简阳市','236'),


-- 237 内江市
('市中区','237'),
('东兴区','237'),
('威远县','237'),
('资中县','237'),
('隆昌县','237'),


-- 238 乐山市
('市中区','238'),
('沙湾区','238'),
('五通桥区','238'),
('金口河区','238'),
('犍为县','238'),
('井研县','238'),
('夹江县','238'),
('沐川县','238'),
('峨边彝族自治县','238'),
('马边彝族自治县','238'),
('峨眉山市','238'),

-- 239 眉山市
('东坡区','239'),
('仁寿县','239'),
('彭山县','239'),
('洪雅县','239'),
('丹棱县','239'),
('青神县','239'),

-- 240 凉山彝族自治州
('西昌市','240'),
('木里藏族自治县','240'),
('盐源县','240'),
('德昌县','240'),
('会理县','240'),
('会东县','240'),
('宁南县','240'),
('普格县','240'),
('布拖县','240'),
('金阳县','240'),
('昭觉县','240'),
('喜德县','240'),
('冕宁县','240'),
('越西县','240'),
('甘洛县','240'),
('美姑县','240'),
('雷波县','240'),


-- 241 雅安市
('雨城区','241'),
('名山县','241'),
('荥经县','241'),
('汉源县','241'),
('石棉县','241'),
('天全县','241'),
('芦山县','241'),
('宝兴县','241'),

-- 242 甘孜藏族自治州
('康定县','242'),
('泸定县','242'),
('丹巴县','242'),
('九龙县','242'),
('雅江县','242'),
('道孚县','242'),
('炉霍县','242'),
('甘孜县','242'),
('新龙县','242'),
('德格县','242'),
('白玉县','242'),
('石渠县','242'),
('色达县','242'),
('理塘县','242'),
('巴塘县','242'),
('乡城县','242'),
('稻城县','242'),
('得荣县','242'),

-- 243 阿坝藏族羌族自治州
('汶川县','243'),
('理县','243'),
('茂县','243'),
('松潘县','243'),
('九寨沟县','243'),
('金川县','243'),
('小金县','243'),
('黑水县','243'),
('马尔康县','243'),
('壤塘县','243'),
('阿坝县','243'),
('若尔盖县','243'),
('红原县','243'),

-- 244 德阳市
('旌阳区','244'),
('中江县','244'),
('罗江县','244'),
('广汉市','244'),
('什邡市','244'),
('绵竹市','244'),

-- 245 广元市
('市中区','245'),
('元坝区','245'),
('朝天区','245'),
('旺苍县','245'),
('青川县','245'),
('剑阁县','245'),
('苍溪县','245'),

-- *********************************************************贵州************************************************************************
-- 246 贵阳市
('南明区','246'),
('云岩区','246'),
('花溪区','246'),
('乌当区','246'),
('白云区','246'),
('小河区','246'),
('开阳县','246'),
('息烽县','246'),
('修文县','246'),
('清镇市','246'),


-- 247 遵义市
('红花岗区','247'),
('汇川区','247'),
('遵义县','247'),
('桐梓县','247'),
('绥阳县','247'),
('正安县','247'),
('道真仡佬族苗族自治县','247'),
('务川仡佬族苗族自治县','247'),
('凤冈县','247'),
('湄潭县','247'),
('余庆县','247'),
('习水县','247'),
('赤水市','247'),
('仁怀市','247'),


-- 248 安顺市
('西秀区','248'),
('平坝县','248'),
('普定县','248'),
('镇宁布依族苗族自治县','248'),
('关岭布依族苗族自治县','248'),
('紫云苗族布依族自治县','248'),

-- 249 黔南布依族苗族自治州
('都匀市','249'),
('福泉市','249'),
('荔波县','249'),
('贵定县','249'),
('瓮安县','249'),
('独山县','249'),
('平塘县','249'),
('罗甸县','249'),
('长顺县','249'),
('龙里县','249'),
('惠水县','249'),
('三都水族自治县','249'),


-- 250 黔东南苗族侗族自治州
('凯里市','250'),
('黄平县','250'),
('施秉县','250'),
('三穗县','250'),
('镇远县','250'),
('岑巩县','250'),
('天柱县','250'),
('锦屏县','250'),
('剑河县','250'),
('台江县','250'),
('黎平县','250'),
('榕江县','250'),
('从江县','250'),
('雷山县','250'),
('麻江县','250'),
('丹寨县','250'),


-- 251 铜仁地区
('铜仁市','251'),
('江口县','251'),
('玉屏侗族自治县','251'),
('石阡县','251'),
('思南县','251'),
('印江土家族苗族自治县','251'),
('德江县','251'),
('沿河土家族自治县','251'),
('松桃苗族自治县','251'),
('万山特区','251'),


-- 252 毕节地区
('毕节市','252'),
('大方县','252'),
('黔西县','252'),
('金沙县','252'),
('织金县','252'),
('纳雍县','252'),
('威宁彝族回族苗族自治县','252'),
('赫章县','252'),


-- 253 六盘水市
('钟山区','253'),
('六枝特区','253'),
('水城县','253'),
('盘县','253'),

-- 254 黔西南布依族苗族自治州
('兴义市','254'),
('兴仁县','254'),
('普安县','254'),
('晴隆县','254'),
('贞丰县','254'),
('望谟县','254'),
('册亨县','254'),
('安龙县','254'),


-- *********************************************************海南************************************************************************
-- 255 海口市
('秀英区','255'),
('龙华区','255'),
('琼山区','255'),
('美兰区','255'),

-- 256 三亚市(无)
-- 257 五指山市(无)
-- 258 琼海市(无)
-- 259 儋州市(无)
-- 260 文昌市(无)
-- 261 万宁市(无)
-- 262 东方市(无)
-- 263 澄迈县(无)
-- 264 定安县(无)
-- 265 屯昌县(无)
-- 266 临高县(无)
-- 267 白沙黎族自治县(无)
-- 268 昌江黎族自治县(无)
-- 269 乐东黎族自治县(无)
-- 270 陵水黎族自治县(无)
-- 271 保亭黎族苗族自治县(无)
-- 272 琼中黎族苗族自治县(无)

-- *********************************************************云南************************************************************************
-- 273 西双版纳傣族自治州
('景洪市','273'),
('勐海县','273'),
('勐腊县','273'),

-- 274 德宏傣族景颇族自治州
('瑞丽市','274'),
('潞西市','274'),
('梁河县','274'),
('盈江县','274'),
('陇川县','274'),

-- 275 昭通市
('昭阳区','275'),
('鲁甸县','275'),
('巧家县','275'),
('盐津县','275'),
('大关县','275'),
('永善县','275'),
('绥江县','275'),
('镇雄县','275'),
('彝良县','275'),
('威信县','275'),
('水富县','275'),

-- 276 昆明市
('五华区','276'),
('盘龙区','276'),
('官渡区','276'),
('西山区','276'),
('东川区','276'),
('呈贡县','276'),
('晋宁县','276'),
('富民县','276'),
('宜良县','276'),
('石林彝族自治县','276'),
('嵩明县','276'),
('禄劝彝族苗族自治县','276'),
('寻甸回族彝族自治县','276'),
('安宁市','276'),

-- 277 大理白族自治州
('大理市','277'),
('漾濞彝族自治县','277'),
('祥云县','277'),
('宾川县','277'),
('弥渡县','277'),
('南涧彝族自治县','277'),
('巍山彝族回族自治县','277'),
('永平县','277'),
('云龙县','277'),
('洱源县','277'),
('剑川县','277'),
('鹤庆县','277'),

-- 278 红河哈尼族彝族自治州
('个旧市','278'),
('开远市','278'),
('蒙自县','278'),
('屏边苗族自治县','278'),
('建水县','278'),
('石屏县','278'),
('弥勒县','278'),
('泸西县','278'),
('元阳县','278'),
('红河县','278'),
('金平苗族瑶族傣族自治县','278'),
('绿春县','278'),
('河口瑶族自治县','278'),

-- 279 曲靖市
('麒麟区','279'),
('马龙县','279'),
('陆良县','279'),
('师宗县','279'),
('罗平县','279'),
('富源县','279'),
('会泽县','279'),
('沾益县','279'),
('宣威市','279'),

-- 280 保山市
('隆阳区','280'),
('施甸县','280'),
('腾冲县','280'),
('龙陵县','280'),
('昌宁县','280'),

-- 281 文山壮族苗族自治州
('文山县','281'),
('砚山县','281'),
('西畴县','281'),
('麻栗坡县','281'),
('马关县','281'),
('丘北县','281'),
('广南县','281'),
('富宁县','281'),

-- 282 玉溪市
('红塔区','282'),
('江川县','282'),
('澄江县','282'),
('通海县','282'),
('华宁县','282'),
('易门县','282'),
('峨山彝族自治县','282'),
('新平彝族傣族自治县','282'),
('元江哈尼族彝族傣族自治县','282'),

-- 283 楚雄彝族自治州
('楚雄市','283'),
('双柏县','283'),
('牟定县','283'),
('南华县','283'),
('姚安县','283'),
('大姚县','283'),
('永仁县','283'),
('元谋县','283'),
('武定县','283'),
('禄丰县','283'),

-- 284 普洱市
('思茅区','284'),
('宁洱哈尼族彝族自治县','284'),
('墨江哈尼族自治县','284'),
('景东彝族自治县','284'),
('景谷傣族彝族自治县','284'),
('镇沅彝族哈尼族拉祜族自治县','284'),
('江城哈尼族彝族自治县','284'),
('孟连傣族拉祜族佤族自治县','284'),
('澜沧拉祜族自治县','284'),
('西盟佤族自治县','284'),
 
-- 285 临沧市
('临翔区','285'),
('凤庆县','285'),
('云县','285'),
('永德县','285'),
('镇康县','285'),
('双江拉祜族佤族布朗族傣族自治县','285'),
('耿马傣族佤族自治县','285'),
('沧源佤族自治县','285'),

-- 286 怒江傈傈族自治州
('泸水县','286'),
('福贡县','286'),
('贡山独龙族怒族自治县','286'),
('兰坪白族普米族自治县','286'),

-- 287 迪庆藏族自治州
('香格里拉县','287'),
('德钦县','287'),
('维西傈僳族自治县','287'),


-- 288 丽江市
('古城区','288'),
('玉龙纳西族自治县','288'),
('永胜县','288'),
('华坪县','288'),
('宁蒗彝族自治县','288'),


-- *********************************************************青海************************************************************************
-- 289 海北藏族自治州
('门源回族自治县','289'),
('祁连县','289'),
('海晏县','289'),
('刚察县','289'),

-- 290 西宁市
('城东区','290'),
('城中区','290'),
('城西区','290'),
('城北区','290'),
('大通回族土族自治县','290'),
('湟中县','290'),
('湟源县','290'),

-- 291 海东地区
('平安县','291'),
('民和回族土族自治县','291'),
('乐都县','291'),
('互助土族自治县','291'),
('化隆回族自治县','291'),
('循化撒拉族自治县','291'),

-- 292 黄南藏族自治州
('同仁县','292'),
('尖扎县','292'),
('泽库县','292'),
('河南蒙古族自治县','292'),

-- 293 海南藏族自治州
('共和县','293'),
('同德县','293'),
('贵德县','293'),
('兴海县','293'),
('贵南县','293'),
-- 294 果洛藏族自治州
('玛沁县','294'),
('班玛县','294'),
('甘德县','294'),
('达日县','294'),
('久治县','294'),
('玛多县','294'),

-- 295 玉树藏族自治州
('玉树县','295'),
('杂多县','295'),
('称多县','295'),
('治多县','295'),
('囊谦县','295'),
('曲麻莱县','295'),

-- 296 海西蒙古族藏族自治州
('格尔木市','296'),
('德令哈市','296'),
('乌兰县','296'),
('都兰县','296'),
('天峻县','296'),

-- *********************************************************陕西************************************************************************
-- 297 西安市
('新城区','297'),
('碑林区','297'),
('莲湖区','297'),
('灞桥区','297'),
('未央区','297'),
('雁塔区','297'),
('阎良区','297'),
('临潼区','297'),
('长安区','297'),
('蓝田县','297'),
('周至县','297'),
('户县','297'),
('高陵县','297'),

-- 298 咸阳市
('秦都区','298'),
('杨陵区','298'),
('渭城区','298'),
('三原县','298'),
('泾阳县','298'),
('乾县','298'),
('礼泉县','298'),
('永寿县','298'),
('彬县','298'),
('长武县','298'),
('旬邑县','298'),
('淳化县','298'),
('武功县','298'),
('兴平市','298'),

-- 299 延安市
('宝塔区','299'),
('延长县','299'),
('延川县','299'),
('子长县','299'),
('安塞县','299'),
('志丹县','299'),
('吴起县','299'),
('甘泉县','299'),
('富县','299'),
('洛川县','299'),
('宜川县','299'),
('黄龙县','299'),
('黄陵县','299'),

-- 300 榆林市
('榆阳区','300'),
('神木县','300'),
('府谷县','300'),
('横山县','300'),
('靖边县','300'),
('定边县','300'),
('绥德县','300'),
('米脂县','300'),
('佳县','300'),
('吴堡县','300'),
('清涧县','300'),
('子洲县','300'),


-- 301 渭南市
('临渭区','301'),
('华县','301'),
('潼关县','301'),
('大荔县','301'),
('合阳县','301'),
('澄城县','301'),
('蒲城县','301'),
('白水县','301'),
('富平县','301'),
('韩城市','301'),
('华阴市','301'),

-- 302 商洛市
('商州区','302'),
('洛南县','302'),
('丹凤县','302'),
('商南县','302'),
('山阳县','302'),
('镇安县','302'),
('柞水县','302'),

-- 303 安康市
('汉滨区','303'),
('汉阴县','303'),
('石泉县','303'),
('宁陕县','303'),
('紫阳县','303'),
('岚皋县','303'),
('平利县','303'),
('镇坪县','303'),
('旬阳县','303'),
('白河县','303'),

-- 304 汉中市
('汉台区','304'),
('南郑县','304'),
('城固县','304'),
('洋县','304'),
('西乡县','304'),
('勉县','304'),
('宁强县','304'),
('略阳县','304'),
('镇巴县','304'),
('留坝县','304'),
('佛坪县','304'),

-- 305 宝鸡市
('渭滨区','305'),
('金台区','305'),
('陈仓区','305'),
('凤翔县','305'),
('岐山县','305'),
('扶风县','305'),
('眉县','305'),
('陇县','305'),
('千阳县','305'),
('麟游县','305'),
('凤县','305'),
('太白县','305'),

-- 306 铜川市
('王益区','306'),
('印台区','306'),
('耀州区','306'),
('宜君县','306'),


-- *********************************************************广西************************************************************************
-- 307 防城港市
('港口区','307'),
('防城区','307'),
('上思县','307'),
('东兴市','307'),

-- 308 南宁市
('兴宁区','308'),
('青秀区','308'),
('江南区','308'),
('西乡塘区','308'),
('良庆区','308'),
('邕宁区','308'),
('武鸣县','308'),
('隆安县','308'),
('马山县','308'),
('上林县','308'),
('宾阳县','308'),
('横县','308'),

-- 309 崇左市
('江洲区','309'),
('扶绥县','309'),
('宁明县','309'),
('龙州县','309'),
('大新县','309'),
('天等县','309'),
('凭祥市','309'),


-- 310 来宾市
('兴宾区','310'),
('忻城县','310'),
('象州县','310'),
('武宣县','310'),
('金秀瑶族自治县','310'),
('合山市','310'),

-- 311 柳州市
('城中区','311'),
('鱼峰区','311'),
('柳南区','311'),
('柳北区','311'),
('柳江县','311'),
('柳城县','311'),
('鹿寨县','311'),
('融安县','311'),
('融水苗族自治县','311'),
('三江侗族自治县','311'),

-- 312 桂林市
('秀峰区','312'),
('叠彩区','312'),
('象山区','312'),
('七星区','312'),
('雁山区','312'),
('阳朔县','312'),
('临桂县','312'),
('灵川县','312'),
('全州县','312'),
('兴安县','312'),
('永福县','312'),
('灌阳县','312'),
('龙胜各族自治县','312'),
('资源县','312'),
('平乐县','312'),
('荔浦县','312'),
('恭城瑶族自治县','312'),

-- 313 梧州市
('万秀区','313'),
('碟山区','313'),
('长洲区','313'),
('苍梧县','313'),
('藤县','313'),
('蒙山县','313'),
('岑溪市','313'),

-- 314 贺州市
('八步区','314'),
('昭平县','314'),
('钟山县','314'),
('富川瑶族自治县','314'),

-- 315 贵港市
('港北区','315'),
('港南区','315'),
('覃塘区','315'),
('平南县','315'),
('桂平市','315'),

-- 316 玉林市
('玉州区','316'),
('容县','316'),
('陆川县','316'),
('博白县','316'),
('兴业县','316'),
('北流市','316'),

-- 317 百色市
('右江区','317'),
('田阳县','317'),
('田东县','317'),
('平果县','317'),
('德保县','317'),
('靖西县','317'),
('那坡县','317'),
('凌云县','317'),
('乐业县','317'),
('田林县','317'),
('西林县','317'),
('隆林各族自治县','317'),

-- 318 钦州市
('钦南区','318'),
('钦北区','318'),
('灵山县','318'),
('浦北县','318'),

-- 319 河池市
('金城江区','319'),
('南丹县','319'),
('天峨县','319'),
('凤山县','319'),
('东兰县','319'),
('罗城仫佬族自治县','319'),
('环江毛南族自治县','319'),
('巴马瑶族自治县','319'),
('都安瑶族自治县','319'),
('大化瑶族自治县','319'),
('宜州市','319'),


-- 320 北海市
('海城区','320'),
('银海区','320'),
('铁山港区','320'),
('合浦县','320'),

-- *********************************************************西藏************************************************************************
-- 321 拉萨市
('城关区','321'),
('林周县','321'),
('当雄县','321'),
('尼木县','321'),
('曲水县','321'),
('堆龙德庆县','321'),
('达孜县','321'),
('墨竹工卡县','321'),

-- 322 日喀则地区
('日喀则市','322'),
('南木林县','322'),
('江孜县','322'),
('定日县','322'),
('萨迦县','322'),
('拉孜县','322'),
('昂仁县','322'),
('谢通门县','322'),
('白朗县','322'),
('仁布县','322'),
('康马县','322'),
('定结县','322'),
('仲巴县','322'),
('亚东县','322'),
('吉隆县','322'),
('聂拉木县','322'),
('萨嘎县','322'),
('岗巴县','322'),

-- 323 山南地区
('乃东县','323'),
('扎囊县','323'),
('贡嘎县','323'),
('桑日县','323'),
('琼结县','323'),
('曲松县','323'),
('措美县','323'),
('洛扎县','323'),
('加查县','323'),
('隆子县','323'),
('错那县','323'),
('浪卡子县','323'),

-- 324 林芝地区
('林芝县','324'),
('工布江达县','324'),
('米林县','324'),
('墨脱县','324'),
('波密县','324'),
('察隅县','324'),
('朗县','324'),


-- 325 昌都地区
('昌都县','325'),
('江达县','325'),
('贡觉县','325'),
('类乌齐县','325'),
('丁青县','325'),
('察雅县','325'),
('八宿县','325'),
('左贡县','325'),
('芒康县','325'),
('洛隆县','325'),
('边坝县','325'),


-- 326 那曲地区
('那曲县','326'),
('嘉黎县','326'),
('比如县','326'),
('聂荣县','326'),
('安多县','326'),
('申扎县','326'),
('索县','326'),
('班戈县','326'),
('巴青县','326'),
('尼玛县','326'),


-- 327 阿里地区
('普兰县','327'),
('札达县','327'),
('噶尔县','327'),
('日土县','327'),
('革吉县','327'),
('改则县','327'),
('措勤县','327'),

-- *********************************************************宁夏************************************************************************
-- 328 银川市
('兴庆区','328'),
('西夏区','328'),
('金凤区','328'),
('永宁县','328'),
('贺兰县','328'),
('灵武市','328'),

-- 329 石嘴山市
('大武口区','329'),
('惠农区','329'),
('平罗县','329'),


-- 330 吴忠市
('利通区','330'),
('盐池县','330'),
('同心县','330'),
('青铜峡市','330'),


-- 331 固原市
('原州区','331'),
('西吉县','331'),
('隆德县','331'),
('泾源县','331'),
('彭阳县','331'),


-- 332 中卫市
('沙坡头区','332'),
('中宁县','332'),
('海原县','332'),

-- *********************************************************新 疆************************************************************************
-- 333 塔城地区
('塔城市','333'),
('乌苏市','333'),
('额敏县','333'),
('沙湾县','333'),
('托里县','333'),
('裕民县','333'),
('和布克赛尔蒙古自治县','333'),

-- 334 哈密地区
('哈密市','334'),
('巴里坤哈萨克自治县','334'),
('伊吾县','334'),

-- 335 和田地区
('和田市','335'),
('和田县','335'),
('墨玉县','335'),
('皮山县','335'),
('洛浦县','335'),
('策勒县','335'),
('于田县','335'),
('民丰县','335'),


-- 336 阿勒泰地区
('阿勒泰市','336'),
('布尔津县','336'),
('富蕴县','336'),
('福海县','336'),
('哈巴河县','336'),
('青河县','336'),
('吉木乃县','336'),


-- 337 克孜勒苏柯尔克孜自治州
('阿图什市','337'),
('阿克陶县','337'),
('阿合奇县','337'),
('乌恰县','337'),


-- 338 博尔塔拉蒙古自治州
('博乐市','338'),
('精河县','338'),
('温泉县','338'),


-- 339 克拉玛依市
('独山子区','339'),
('克拉玛依区','339'),
('白碱滩区','339'),
('乌尔禾区','339'),

-- 340 乌鲁木齐市
('天山区','340'),
('沙依巴克区','340'),
('新市区','340'),
('水磨沟区','340'),
('头屯河区','340'),
('达坂城区','340'),
('米东区','340'),
('乌鲁木齐县','340'),

-- 341 石河子市(无)
-- 342 昌吉回族自治州
('昌吉市','342'),
('阜康市','342'),
('呼图壁县','342'),
('玛纳斯县','342'),
('奇台县','342'),
('吉木萨尔县','342'),
('木垒哈萨克自治县','342'),

-- 343 五家渠市(无)
-- 344 吐鲁番地区
('吐鲁番市','344'),
('鄯善县','344'),
('托克逊县','344'),

-- 345 巴音郭楞蒙古自治州
('库尔勒市','345'),
('轮台县','345'),
('尉犁县','345'),
('若羌县','345'),
('且末县','345'),
('焉耆回族自治县','345'),
('和静县','345'),
('和硕县','345'),
('博湖县','345'),


-- 346 阿克苏地区
('阿克苏市','346'),
('温宿县','346'),
('库车县','346'),
('沙雅县','346'),
('新和县','346'),
('拜城县','346'),
('乌什县','346'),
('阿瓦提县','346'),
('柯坪县','346'),

-- 347 阿拉尔市(无)
-- 348 喀什地区
('喀什市','348'),
('疏附县','348'),
('疏勒县','348'),
('英吉沙县','348'),
('泽普县','348'),
('莎车县','348'),
('叶城县','348'),
('麦盖提县','348'),
('岳普湖县','348'),
('伽师县','348'),
('巴楚县','348'),
('塔什库尔干塔吉克自治县','348'),

-- 349 图木舒克市(无)
-- 350 伊犁哈萨克自治州
('伊宁市','350'),
('奎屯市','350'),
('伊宁县','350'),
('察布查尔锡伯自治县','350'),
('霍城县','350'),
('巩留县','350'),
('新源县','350'),
('昭苏县','350'),
('特克斯县','350'),
('尼勒克县','350'),


-- *********************************************************内蒙古************************************************************************
-- 351 呼伦贝尔市
('海拉尔区','351'),
('阿荣旗','351'),
('莫力达瓦达斡尔族自治旗','351'),
('鄂伦春自治旗','351'),
('鄂温克族自治旗','351'),
('陈巴尔虎旗','351'),
('新巴尔虎左旗','351'),
('新巴尔虎右旗','351'),
('满洲里市','351'),
('牙克石市','351'),
('扎兰屯市','351'),
('额尔古纳市','351'),
('根河市','351'),

-- 352 呼和浩特市
('新城区','352'),
('回民区','352'),
('玉泉区','352'),
('赛罕区','352'),
('土默特左旗','352'),
('托克托县','352'),
('和林格尔县','352'),
('清水河县','352'),
('武川县','352'),

-- 353 包头市
('东河区','353'),
('昆都仑区','353'),
('青山区','353'),
('石拐区','353'),
('白云鄂博矿区','353'),
('九原区','353'),
('土默特右旗','353'),
('固阳县','353'),
('达尔罕茂明安联合旗','353'),

-- 354 乌海市
('海勃湾区','354'),
('海南区','354'),
('乌达区','354'),

-- 355 乌兰察布市
('集宁区','355'),
('卓资县','355'),
('化德县','355'),
('商都县','355'),
('兴和县','355'),
('凉城县','355'),
('察哈尔右翼前旗','355'),
('察哈尔右翼中旗','355'),
('察哈尔右翼后旗','355'),
('四子王旗','355'),
('丰镇市','355'),

-- 356 通辽市
('科尔沁区','356'),
('科尔沁左翼中旗','356'),
('科尔沁左翼后旗','356'),
('开鲁县','356'),
('库伦旗','356'),
('奈曼旗','356'),
('扎鲁特旗','356'),
('霍林郭勒市','356'),

-- 357 赤峰市
('红山区','357'),
('元宝山区','357'),
('松山区','357'),
('阿鲁科尔沁旗','357'),
('巴林左旗','357'),
('巴林右旗','357'),
('林西县','357'),
('克什克腾旗','357'),
('翁牛特旗','357'),
('喀喇沁旗','357'),
('宁城县','357'),
('敖汉旗','357'),


-- 358 鄂尔多斯市
('东胜区','358'),
('达拉特旗','358'),
('准格尔旗','358'),
('鄂托克前旗','358'),
('鄂托克旗','358'),
('杭锦旗','358'),
('乌审旗','358'),
('伊金霍洛旗','358'),

-- 359 巴彦淖尔市
('临河区','359'),
('五原县','359'),
('磴口县','359'),
('乌拉特前旗','359'),
('乌拉特中旗','359'),
('乌拉特后旗','359'),
('杭锦后旗','359'),

-- 360 锡林郭勒盟
('二连浩特市','360'),
('锡林浩特市','360'),
('阿巴嘎旗','360'),
('苏尼特左旗','360'),
('苏尼特右旗','360'),
('东乌珠穆沁旗','360'),
('西乌珠穆沁旗','360'),
('太仆寺旗','360'),
('镶黄旗','360'),
('正镶白旗','360'),
('正蓝旗','360'),
('多伦县','360'),

-- 361 兴安盟
('乌兰浩特市','361'),
('阿尔山市','361'),
('科尔沁右翼前旗','361'),
('科尔沁右翼中旗','361'),
('扎赉特旗','361'),
('突泉县','361'),

-- 362 阿拉善盟
('阿拉善左旗','362'),
('阿拉善右旗','362'),
('额济纳旗','362');

-- *********************************************************港澳台************************************************************************
-- 363 台北市(无)
-- 364 高雄市(无)
-- 365 基隆市(无)
-- 366 台中市(无)
-- 367 台南市(无)
-- 368 新竹市(无)
-- 369 嘉义市(无)
-- 370 澳门特别行政区(无)

GRANT ALL ON NIMBUS_IPAD.* TO aipad@'%' IDENTIFIED BY 'aipadpass';
FLUSH PRIVILEGES;

CREATE DATABASE NIMBUS_SOON;

########################################################################################################

CREATE TABLE NIMBUS_SOON.SOON
(
	ID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	EMAIL NVARCHAR(41) NOT NULL,
	NAME NVARCHAR(41) NOT NULL,

	PRIMARY KEY (ID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

########################################################################################################

CREATE TABLE NIMBUS_SOON.TESTCODE
(
	ID INT UNSIGNED NOT NULL AUTO_INCREMENT,
	CODE NVARCHAR(9) NOT NULL,

	PRIMARY KEY (ID)
) DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

CREATE INDEX CODE_INDEX ON NIMBUS_SOON.TESTCODE (CODE(8));

########################################################################################################

GRANT ALL ON NIMBUS_SOON.* TO asoon@'%' IDENTIFIED BY 'asoonpass';
FLUSH PRIVILEGES;